...
def _collect_board_map_totals() -> dict:
    ...
    return totals
def _collect_board_totals_snapshot() -> dict:
    board_totals = {
        "active_users": 0,
        "shadow_mutes": 0,
        "regular_mutes": 0,
...
def _collect_queues_snapshot(queue_sizes: dict, top_queues: list, queue_age_summary: dict) -> dict:
    return {
...
def _collect_delivery_priority_snapshot(priority_counts: dict) -> dict:
    return {
...
def _collect_anime_media_snapshot() -> dict:
    return {
...
def _collect_mode_punchup_snapshot() -> dict:
    return {
...
def _collect_contextual_replies_snapshot() -> dict:
    return {
...
def _collect_maps_snapshot(pending_done: int) -> dict:
    return {
...
def _collect_runtime_snapshot() -> dict:
    ...
    return {
        ...
        "queues": _collect_queues_snapshot(queue_sizes, top_queues, queue_age_summary),
        "delivery_priority": _collect_delivery_priority_snapshot(priority_counts),
        "recipients": _recipient_counts_snapshot(),
        "durable_delivery": {
            "enabled": DURABLE_DELIVERY_QUEUE_ENABLED,
            **durable_delivery_stats,
        },
        "anime_media": _collect_anime_media_snapshot(),
        "mode_punchup": _collect_mode_punchup_snapshot(),
        "contextual_replies": _collect_contextual_replies_snapshot(),
        "reply_coverage": {
            "updated_at": reply_coverage_updated_at,
            **reply_coverage_stats,
        },
        "delivery": _summarize_delivery_metrics(),
        "maps": _collect_maps_snapshot(pending_done),
        "board_maps": _collect_board_map_totals(),
        "board_totals": _collect_board_totals_snapshot(),

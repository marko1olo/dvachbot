...
def _collect_board_totals() -> dict:
    ...
    for board_id in BOARDS:
        ...
        board_totals["threads"] += _safe_len(get_threads_data(board_id))
...
async def global_error_handler(event: types.ErrorEvent) -> bool:
    ...
    if isinstance(exception, TelegramForbiddenError):
        await _handle_telegram_forbidden_error(update)
        return True
...
async def log_memory_summary():
    ...
    for board_id in BOARDS:
        b_data = board_data[board_id]
        current_stats[f"board[{board_id}].threads"] = len(get_threads_data(board_id))
...
def _sync_collect_board_statistics(hour_ago_ts, posts_meta_list):
    ...
    for post_time, b_id in posts_meta_list:
        ...
        if post_time and post_time >= hour_ago_ts and b_id:
...
async def board_statistics_broadcaster():
    ...
    while True:
        try:
            ...
            hour_ago_ts = (now - timedelta(hours=1)).timestamp()
...
class ModeTransformer:
    def __init__(self, content: dict, board_id: str):
        ...
        self.modified_content = content.copy()
        self.active_mode_key = next((mode for mode in MODE_FLAGS if self.b_data.get(mode)), None)
        self.text_key = None
        self.plain_text = ""
    ...
    async def _run_transformation(self):
        loop = asyncio.get_running_loop()
        if self.b_data.get('schizo_mode'):
            return await loop.run_in_executor(None, shizo_transform, self.plain_text, self.header)
        elif self.b_data.get('gopnik_mode'):
            return await loop.run_in_executor(None, gopnik_transform, self.plain_text)
        elif self.b_data.get('imperial_mode'):
            return await loop.run_in_executor(None, imperial_transform, self.plain_text, self.header)
        elif self.b_data.get('warhammer_mode'):
            return await loop.run_in_executor(None, warhammer_transform, self.plain_text, self.header)
        elif self.b_data.get('polish_mode'):
    ...
    def _handle_transform_result(self, transform_result):
        if transform_result and isinstance(transform_result, tuple):
            ...
            if res_type == 'image' and self.allow_visual:
                ...
                self.modified_content[self.text_key] = transformed_text
                self.plain_text = transformed_text
        return False

    async def _apply_residual_modes(self):
        loop = asyncio.get_running_loop()
        transformed_text = self.plain_text
        if self.b_data.get('zaputin_mode'):
            transformed_text = await loop.run_in_executor(None, zaputin_transform, transformed_text)
        elif self.b_data.get('suka_blyat_mode'):
    ...
    async def _apply_anime_mode(self):
        loop = asyncio.get_running_loop()
        if self.b_data.get('anime_mode'):
            current_text = self.modified_content.get(self.text_key, '')
            transformed_plain_text = await loop.run_in_executor(None, anime_transform, current_text)
            transformed_plain_text = await _maybe_punch_up_text(transformed_plain_text, 'anime_mode', self.board_id)
            self.modified_content[self.text_key] = escape_html(transformed_plain_text)

            if self.modified_content.get('type') == 'text' and random.random() < 0.33:
                ...
                try:
                    ...
                    anime_img_url = None
                if anime_img_url:
                    text_content = self.modified_content.pop('text', '')
                    self.modified_content.update({'type': 'photo', 'caption': text_content, 'image_url': anime_img_url})
        elif self.active_mode_key:
            current_text = self.modified_content.get(self.text_key, '')
            if current_text:
                self.modified_content[self.text_key] = await _maybe_punch_up_text(current_text, self.active_mode_key, self.board_id)

    async def apply(self) -> dict:
        if not self._check_active_mode():
            return self.modified_content
        if not self._determine_text_key():
            return self.modified_content

        transform_result = await self._run_transformation()
        is_visual_applied = self._handle_transform_result(transform_result)
        if is_visual_applied:
            return self.modified_content
...
async def cmd_global_pin(message: types.Message, board_id: str | None, stream: str = 'ru'):
    ...
    if lang == 'en':
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(text="🛒 Market (/shop)", callback_data="shop_main_hub"),
                InlineKeyboardButton(text="💼 Jobs (/work)", callback_data="work_main_hub")
            ],
            [
                InlineKeyboardButton(text="🎰 Casino (/casino)", callback_data="casino_main_hub"),
                InlineKeyboardButton(text="📦 Lootboxes (/lootbox)", callback_data="shop_cat_lootbox")
            ],
            [
                InlineKeyboardButton(text="🎭 Character (/avatar)", callback_data="avatar_view"),
                InlineKeyboardButton(text="🏆 Trophies (/ach)", callback_data="achievements_view")
            ],
            [
                InlineKeyboardButton(text="💸 Withdraw", callback_data="start_withdrawal"),
                InlineKeyboardButton(text="📊 Rates", callback_data="scam_rates")
            ],
            [
                InlineKeyboardButton(text="🤝 Invite Friend (+1488 ₪)", switch_inline_query=f"\nJoin Tgach! Referral: {ref_link}")
            ]
        ])
    else:
        kb = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [
                ...
                InlineKeyboardButton(text="📦 Лутбоксы (/lootbox)", callback_data="shop_cat_lootbox")
            ],
            [
            ...
            [
                ...
                InlineKeyboardButton(text="📊 Курс биржи", callback_data="scam_rates")
            ],
            [
                InlineKeyboardButton(text="🤝 Пригласить друга (+1488 ₪)", switch_inline_query=f"\nЗаходи в Тгач! За каждого друга начисляют +1488 шекелей и платят за реакции! Моя ссылка: {ref_link}")
            ]
        ])
...
# ══════════════════════════════════════════════════════════════════════════════
# 🚨 FAST RESCUE: Мгновенное спасение и самооборона из уведомлений
# ══════════════════════════════════════════════════════════════════════════════
@dp.callback_query(F.data.startswith("fast_rescue:"))
async def cb_fast_rescue(callback: types.CallbackQuery, board_id: str | None):
    if not board_id: return
    user_id = callback.from_user.id
...
@dp.callback_query(F.data.in_(["work_main_hub", "work_hub"]))
async def cb_work_main_hub(callback: types.CallbackQuery, board_id: str | None):
    if not board_id: return
    user_id = callback.from_user.id
    text, kb = await _build_work_card(user_id, board_id)
    try:
...
# =============================================================================
# 🎰 ПОДПОЛЬНОЕ КАЗИНО ТГАЧА & 💸 ДРОП ШЕКЕЛЕЙ В ТРЕД (ЧЕКИ НА РЕАКЦИЮ)
# =============================================================================
import casino_engine
import drop_engine

@dp.message(Command("drop", "выбросить", "дроп", "чек", "throw", "giveaway"))
async def cmd_drop(message: types.Message, board_id: str | None, stream: str = 'ru'):
    """Команда /drop <сумма> — выбрасывает шекели в тред для быстрейшего клика."""
    if not board_id: return
    user_id = message.from_user.id
    db = await get_pool()
    balance = await get_user_global_balance(db, user_id)
    ...
    if len(parts) > 1 and parts[1].isdigit():
        ...
        kb = drop_engine.get_drop_claim_keyboard(drop_rec.drop_id, amount)
        caption = (
            f"💸 <b>ДРОП ШЕКЕЛЕЙ В ТРЕД!</b>\n\n"
            f"👤 <b>{donor_name}</b> выбросил <b>{amount} ₪</b> прямо в тред!\n"
            f"⚡ Кто первый нажмет на кнопку ниже — забирает весь куш себе!\n\n"
            f"⏳ <i>Дроп сгорит и вернется анону через 10 минут, если никто не заберет.</i>"
        )
        from banner_manager import send_banner_message
        donor_msg = await send_banner_message(
            bot=message.bot,
            chat_id=message.chat.id,
            caption=caption,
            reply_markup=kb,
    ...
    kb = drop_engine.get_drop_creator_keyboard(int(balance))
    caption = (
        f"💸 <b>ДРОП ШЕКЕЛЕЙ В ТРЕД (ЧЕК НА СКОРОСТЬ)</b>\n\n"
        f"💳 Твой текущий баланс: <code>{int(balance)} ₪</code>\n"
        f"Выбери, сколько шекелей ты хочешь выбросить в тред для всех анонов:\n"
        f"<i>(Или введи команду с суммой, например: <code>/drop 500</code>)</i>"
    )
    from banner_manager import send_banner_message
    await send_banner_message(
        bot=message.bot,
...
async def cb_drop_handler(callback: types.CallbackQuery, board_id: str | None):
    ...
    if action == "claim":
        ...
        new_text = (
            ...
            f"🏆 Победитель: <b>{claimer_name}</b> оказался самым быстрым и забрал весь куш!\n\n"
            f"⚡ <i>Дроп закрыт.</i>"
        )
        ...
        if drop_rec.donor_id and drop_rec.donor_id != user_id:
            try:
                await callback.bot.send_message(
                    chat_id=drop_rec.donor_id,
                    text=f"💸 <b>ТВОЙ ДРОП ЗАБРАЛИ!</b>\n\nАнон <b>{claimer_name}</b> только что перехватил твой дроп на <b>{int(drop_rec.amount)} ₪</b>!",
                    parse_mode="HTML"
                )
            except Exception:
                pass
        ...
        await callback.answer("✅ Дроп отправлен в тред!")
        kb = drop_engine.get_drop_claim_keyboard(drop_rec.drop_id, amount)
        caption = (
            f"💸 <b>ДРОП ШЕКЕЛЕЙ В ТРЕД!</b>\n\n"
            f"👤 <b>{donor_name}</b> выбросил <b>{amount} ₪</b> прямо в тред!\n"
...
async def cmd_slots(message: types.Message, board_id: str | None, stream: str = 'ru'):
    if not board_id: return
    user_id = message.from_user.id
    parts = (message.text or "").strip().split()
    bet = int(parts[1]) if len(parts) > 1 and parts[1].isdigit() else 100
    bet = max(10, min(1000000, bet))
    await _show_slots_lobby(message.bot, message.chat.id, user_id, board_id, bet)
    try:
        await message.delete()
    except Exception:
        pass
...
async def _show_slots_lobby(bot, chat_id: int, user_id: int, board_id: str, bet: int = 100, message_to_edit=None):
    db = await get_pool()
    async with db_lock:
        balance = await get_user_global_balance(db, user_id)
...
@dp.message(Command("coinflip", "flip", "монетка", "монета", "орел", "решка"))
async def cmd_coinflip(message: types.Message, board_id: str | None, stream: str = 'ru'):
    if not board_id: return
    user_id = message.from_user.id
    parts = (message.text or "").strip().split()
    bet = int(parts[1]) if len(parts) > 1 and parts[1].isdigit() else 100
    bet = max(10, min(1000000, bet))
    await _show_coinflip_lobby(message.bot, message.chat.id, user_id, board_id, bet)
    try:
        await message.delete()
    except Exception:
        pass
...
async def _show_coinflip_lobby(bot, chat_id: int, user_id: int, board_id: str, bet: int = 100, message_to_edit=None):
    db = await get_pool()
    async with db_lock:
        balance = await get_user_global_balance(db, user_id)
    ...
    caption = (
        ...
        f"💰 Выбранная ставка: <code>{bet} ₪</code>\n\n"
        f"🎯 <b>Правила:</b>\n"
        f"• Шанс победы: <b>50%</b>\n"
        f"• Коэффициент выигрыша: <b>x1.95</b>\n"
        f"• Угадай, какой стороной выпадет шекель — 🦅 Орёл или 👑 Решка!\n\n"
        f"👇 Выбери ставку и сторону для броска:"
    )
    if message_to_edit:
        try:
            await message_to_edit.edit_caption(caption=caption, reply_markup=kb, parse_mode="HTML")
...
@dp.message(Command("blackjack", "bj", "блэкджек", "21", "очко"))
async def cmd_blackjack(message: types.Message, board_id: str | None, stream: str = 'ru'):
    if not board_id: return
    user_id = message.from_user.id
    parts = (message.text or "").strip().split()
    bet = int(parts[1]) if len(parts) > 1 and parts[1].isdigit() else 100
    bet = max(10, min(1000000, bet))
    await _show_blackjack_lobby(message.bot, message.chat.id, user_id, board_id, bet)
    try:
        await message.delete()
    except Exception:
        pass
...
async def _show_blackjack_lobby(bot, chat_id: int, user_id: int, board_id: str, bet: int = 100, message_to_edit=None):
    db = await get_pool()
    async with db_lock:
        balance = await get_user_global_balance(db, user_id)
    ...
    caption = (
        ...
        f"💰 Выбранная ставка: <code>{bet} ₪</code>\n\n"
        f"🎯 <b>Правила:</b>\n"
        f"• Цель: набрать больше очков, чем дилер, не превысив 21.\n"
        f"• Карты 2–10 = по номиналу, J/Q/K = 10, Туз = 11 или 1.\n"
        f"• Натуральный Блэкджек (21 с первых 2 карт) платит <b>x2.5 (3:2)</b>!\n"
        f"• Обычная победа платит <b>x2.0</b>. Дилер добирает до 17.\n"
        f"• Доступно удвоение (Дабл x2) на первом ходе.\n\n"
        f"👇 Выбери ставку и нажми «Раздать карты»:"
    )
    if message_to_edit:
        try:
            await message_to_edit.edit_caption(caption=caption, reply_markup=kb, parse_mode="HTML")
...
async def _start_blackjack_game(bot, chat_id: int, user_id: int, board_id: str, bet: int):
    ...
    async with casino_engine.session_lock:
        casino_engine.active_bj_sessions[user_id] = {
            "bet": bet,
            "deck": deck,
    ...
    caption = (
        f"🃏 <b>БЛЭКДЖЕК: ИДЕТ ИГРА</b>\n\n"
        f"👤 Твоя рука: {casino_engine.format_hand(player_hand)} (<b>{player_score}</b>)\n"
        f"🎩 Дилер: {casino_engine.format_hand(dealer_hand, hide_dealer_card=True)}\n\n"
...
async def cmd_russian_roulette(message: types.Message, board_id: str | None, stream: str = 'ru'):
    if not board_id: return
    user_id = message.from_user.id
    parts = (message.text or "").strip().split()
    bet = int(parts[1]) if len(parts) > 1 and parts[1].isdigit() else 100
...
async def _show_roulette_lobby(bot, chat_id: int, user_id: int, board_id: str, bet: int = 100, message_to_edit=None):
    db = await get_pool()
    async with db_lock:
        balance = await get_user_global_balance(db, user_id)
    ...
    caption = (
        ...
        f"💳 Твой баланс: <code>{int(balance)} ₪</code>\n"
        f"💰 Начальная ставка: <code>{bet} ₪</code>\n\n"
        f"🎯 <b>Правила:</b>\n"
        f"• Барабан на 6 камор, 1 боевой патрон (16.67% летальность).\n"
        f"• Каждый успешный спуск увеличивает твой куш:\n"
        f"  1 выстрел: <b>x1.25</b> | 2 выстрела: <b>x1.50</b>\n"
        f"  3 выстрела: <b>x2.00</b> | 4 выстрела: <b>x3.00</b> | 5 выстрелов: <b>x5.00+</b>\n"
        f"• В любой момент после выстрела можно нажать <b>Забрать куш</b>.\n\n"
        f"👇 Выбери ставку и нажми «Взвести курок»:"
    )
    if message_to_edit:
        try:
            await message_to_edit.edit_caption(caption=caption, reply_markup=kb, parse_mode="HTML")
...
async def _execute_russian_roulette_shot(bot, chat_id: int, user_id: int, board_id: str, bet: int):
    ...
    # Survived
    kb = casino_engine.get_roulette_keyboard(bet, mult)
    win_potential = int(bet * mult)
    caption = (
        f"💀 <b>РУССКАЯ РУЛЕТКА: ВЫЖИВАНИЕ</b>\n\n"
        f"<b>{status}</b>\n\n"
        f"• Начальная ставка: <code>{bet} ₪</code>\n"
        f"• Текущий куш: <b>+{win_potential} ₪</b> (Множитель: <code>x{mult:.2f}</code>)\n"
...
async def cb_casino_handler(callback: types.CallbackQuery, board_id: str | None):
    if not board_id: return
    user_id = callback.from_user.id
    ...
    if section == "hub":
        ...
        if game == "slots":
            await _show_slots_lobby(callback.bot, callback.message.chat.id, user_id, board_id, 100, message_to_edit=callback.message)
        elif game == "coin":
            await _show_coinflip_lobby(callback.bot, callback.message.chat.id, user_id, board_id, 100, message_to_edit=callback.message)
        elif game == "bj":
            await _show_blackjack_lobby(callback.bot, callback.message.chat.id, user_id, board_id, 100, message_to_edit=callback.message)
        elif game == "roulette":
        ...
        if action == "lobby":
            ...
            await _show_slots_lobby(callback.bot, callback.message.chat.id, user_id, board_id, bet, message_to_edit=callback.message)
        ...
        side_or_action = parts[2] if len(parts) > 2 else "heads"
        if side_or_action == "lobby":
            bet = int(parts[3]) if len(parts) > 3 and parts[3].isdigit() else 100
            await _show_coinflip_lobby(callback.bot, callback.message.chat.id, user_id, board_id, bet, message_to_edit=callback.message)
            await callback.answer(f"Ставка: {bet} ₪")
            ...
            try:
                ...
                pass
            await callback.answer(f"Ставка изменена на {bet} ₪")

    elif section == "bj":
        action = parts[2] if len(parts) > 2 else ""
        if action == "lobby":
            bet = int(parts[3]) if len(parts) > 3 and parts[3].isdigit() else 100
            await _show_blackjack_lobby(callback.bot, callback.message.chat.id, user_id, board_id, bet, message_to_edit=callback.message)
            await callback.answer(f"Ставка: {bet} ₪")
            return
        elif action == "start":
            bet = int(parts[3]) if len(parts) > 3 and parts[3].isdigit() else 100
            await _start_blackjack_game(callback.bot, callback.message.chat.id, user_id, board_id, bet)
            await callback.answer()
            return
        ...
        if action == "hit":
            ...
            p_score = casino_engine.calculate_hand(p_hand)
            ...
            # Still in game
            kb = casino_engine.get_blackjack_keyboard(bet, can_double=False)
            caption = (
                f"🃏 <b>БЛЭКДЖЕК: ИДЕТ ИГРА</b>\n\n"
                f"👤 Твоя рука: {casino_engine.format_hand(p_hand)} (<b>{p_score}</b>)\n"
                f"🎩 Дилер: {casino_engine.format_hand(d_hand, hide_dealer_card=True)}\n\n"
                f"• Ставка: <code>{bet} ₪</code>\n"
                f"👇 Твой ход: взять карту или остановиться?"
            )
            try:
            ...
            while casino_engine.calculate_hand(d_hand) < 17:
            ...
            caption = (
                ...
                f"👤 Твоя рука: {casino_engine.format_hand(p_hand)} (<b>{p_score}</b>)\n"
                f"🎩 Дилер: {casino_engine.format_hand(d_hand)} (<b>{d_score}</b>)\n\n"
                f"<b>{result_title}</b>\n"
        ...
        if action == "lobby":
            bet = int(parts[3]) if len(parts) > 3 and parts[3].isdigit() else 100
            await _show_roulette_lobby(callback.bot, callback.message.chat.id, user_id, board_id, bet, message_to_edit=callback.message)
            await callback.answer(f"Ставка: {bet} ₪")
            return
...
async def cb_prof_card(callback: types.CallbackQuery, board_id: str | None, stream: str = 'ru'):
    ...
    try:
        ...
        runtime_logger.warning("cb_prof_card: failed regenerating stats card: %s", e)
...
async def cmd_start(message: types.Message, state: FSMContext, board_id: str | None, stream: str = 'ru'):
    ...
    if board_id in THREAD_BOARDS:
        ...
        if command_payload and command_payload.startswith("thread_"):
            ...
            b_data = board_data[board_id]
            if thread_id in get_threads_data(board_id):
...
class StackedAnimeHandler:
    ...
    async def process(self):
        ...
        self._log_if_capped(raw_requested_count, requested_count)

        if await self._check_strict_daily_limit(requested_count):
            return

        if await self._check_hourly_limit(requested_count):
            return

        self._increment_hourly_count(requested_count)

        if self.board_id == 'b' and await self._check_board_spam_limit():
            return

        fetcher_tasks, command_counts = self._build_fetcher_tasks()
        if not fetcher_tasks:
            return

        if self.board_id == 'b':
            self._update_board_spam_tracker(len(fetcher_tasks))
    ...
    def _reset_hourly_counters(self):
        if self.current_time - user_hourly_image_reset[self.user_id] > 3600:
            user_hourly_image_count[self.user_id] = 0
            user_hourly_image_reset[self.user_id] = self.current_time

    def _calculate_requested_count(self) -> int:
        raw_requested_count = 0
        for _, num_no_space, num_with_space in self.matches:
            count = 1
    ...
    def _log_if_capped(self, raw_requested_count: int, requested_count: int):
        if raw_requested_count > self.max_images_for_board:
            import json
            runtime_logger.warning(
    ...
    async def _check_strict_daily_limit(self, requested_count: int) -> bool:
        if self.user_id in self.b_data.get('anime_strict_limits', set()):
            tracker = self.b_data['anime_daily_tracker'][self.user_id]
            if self.current_time > tracker['reset_at']:
                tracker['count'] = 0
                tracker['reset_at'] = self.current_time + 86400
            ...
            if tracker['count'] + requested_count > 10:
                ...
                try:
                    sent = await self.message.answer(msg)
    ...
    async def _check_hourly_limit(self, requested_count: int) -> bool:
        if user_hourly_image_count[self.user_id] + requested_count > HOURLY_IMAGE_LIMIT:
            if self.lang == 'en': phrases = ANIME_HOURLY_LIMIT_PHRASES['en']
            elif self.lang == 'jp': phrases = ANIME_HOURLY_LIMIT_PHRASES['jp']
            else: phrases = ANIME_HOURLY_LIMIT_PHRASES['ru']

            import random
            limit_msg = random.choice(phrases)
            try:
                sent = await self.message.answer(limit_msg)
                spawn_task(delete_message_after_delay(sent, 15))
                await self.message.delete()
            except Exception:
                pass
    ...
    def _increment_hourly_count(self, requested_count: int):
        user_hourly_image_count[self.user_id] += requested_count

    async def _check_board_spam_limit(self) -> bool:
        total_requested_images = self._calculate_requested_count()
        total_requested_images = min(total_requested_images, self.max_images_for_board)
        if not check_image_spam_limit(self.board_id, total_requested_images):
            if self.lang == 'en':
            ...
            # HARDCODED limits for the string since we stripped them from globals
            part2 = random.choice(phrases_spam).format(limit=30, minutes=300 // 60)
            cooldown_msg = f"{part1}\n\n{part2}"
            try:
                sent_msg = await self.message.answer(cooldown_msg)
    ...
    def _build_fetcher_tasks(self):
        ...
        for command, num_no_space, num_with_space in self.matches:
            ...
            for _ in range(count):
                if len(fetcher_tasks) < self.max_images_for_board:
    ...
    def _update_board_spam_tracker(self, task_count: int):
        update_image_spam_tracker(self.board_id, task_count)
    ...
    def _generate_caption(self, command_counts: dict) -> str:
        ...
        if not final_caption and random.random() < 0.30 and command_counts:
            ...
            if chosen_category == 'fap':
                ...
                elif self.lang == 'jp': phrase_list = FAP_SUCCESS_PHRASES_JP
                else: phrase_list = FAP_SUCCESS_PHRASES
            elif chosen_category == 'gatari':
                if self.lang == 'en': phrase_list = GATARI_SUCCESS_PHRASES_EN
                elif self.lang == 'jp': phrase_list = GATARI_SUCCESS_PHRASES_JP
...
async def handle_stacked_anime_commands(message: types.Message, board_id: str | None, stream: str = 'ru'):
    ...
    handler = StackedAnimeHandler(message, board_id, stream)
...
MODE_END_PHRASES = {
    ...
    'rus_mode': RUS_PHRASES_END,
    'abu_mode': ABU_PHRASES_END,
}
...
@dp.message(Command("matrix", "matrica", "matriza", "redpill", "neo", "матрица", "нео"))
...
@dp.message(Command("america", "usa", "liberty", "freedom", "америка", "сша", "техас"))
...
@dp.message(Command("holiday", "newyear", "xmas", "christmas", "ny", "новыйгод", "праздник", "нг"))
...
@dp.message(Command("oldweb", "oldnet", "icq", "winamp", "forum", "2007", "emo", "олдвеб", "аська", "упячка"))
...
@dp.message(Command("jewish", "talmud", "odessa", "shabbat", "rabbi", "evrei", "evrey", "еврей", "одесса", "талмуд", "гешефт"))
...
async def cmd_del(message: types.Message, board_id: str | None, stream: str = 'ru'):
    ...
    if is_janitor and not admin_status:
        post_author_id = 0
        post_ts = 0
        if post_num:
            async with storage_lock:
                if post_num in messages_storage:
                    p_data = messages_storage[post_num]
                    post_author_id = p_data.get('author_id', 0)
                    post_ts = p_data.get('timestamp', 0)
            if not post_ts or not post_author_id:
                async with db.execute("SELECT author_id, timestamp FROM Posts WHERE post_num = ?", (post_num,)) as cur:
                    p_row = await cur.fetchone()
                    if p_row:
                        post_author_id = p_row[0]
                        post_ts = p_row[1]

        now = time.time()
        # Ограничение возраста: максимум 15 минут (900 сек)
        if post_ts and (now - post_ts > 900):
        ...
        # Защита фольгой
        if post_author_id:
            target_items = await _get_user_active_items(db, post_author_id, board_id)
            if target_items.get("tinfoil_hat", 0) > now:
                destroyed, left_h, left_m, _ = apply_tinfoil_damage(target_items, int(now), hours_damage=4.0, burn_chance=0.15)
                async with db_lock:
                    await db.execute("UPDATE Users SET active_items = ? WHERE user_id = ? AND board_id = ?",
                                     (json.dumps(target_items), post_author_id, board_id))
                    await db.commit()
                await message.answer("👽 <b>ШАПОЧКА ИЗ ФОЛЬГИ!</b>\nУ автора поста надета Шапочка из фольги! Метла дворника соскользнула, пост защищён от удаления.", parse_mode="HTML")
                try: await message.delete()
                except Exception: pass
                return

        # Списываем 1 попытку только при успешном удалении
        async with db_lock:
...
async def cmd_poll(message: types.Message, state: FSMContext, board_id: str | None, stream: str = 'ru'):
    ...
    if now - last_poll_creation_time[user_id] < 60:
        ...
        try:
            if lang == 'en':
                ...
                import random
                cooldown_msg = random.choice([
                    "⏳ Эй, спермобак, опросы раз в минуту. Остынь.",
                    "⏳ Куда спамишь, уебок? Кулдаун на опросы - 60 сек.",
                    "⏳ Жди минуту, шизоид. Твои тупые опросы могут и подождать."
                ])
    ...
    if message.reply_to_message and message.reply_to_message.media_group_id:
        ...
        if lang == 'en':
            ...
            error_text = random.choice([
                "Медиагруппы в опросах не пашут, долбоеб. Прикрепи одну пикчу.",
                "Ты ебанутый? Зачем ты суешь сюда альбом? Выбери одну картинку.",
                "Альбомы для опросов не работают, шиз. Скинь одно видео или фото."
            ])
        try:
            await message.answer(error_text)
        except (TelegramForbiddenError, TelegramBadRequest):
    ...
    if not question_text or len(options) < 2 or len(options) > 5:
        ...
        if lang == 'en':
            ...
            import random
            errs = [
                "<b>Ты долбоеб или да? Формат неверный!</b>\nЮзай `|` между вопросом и ответами.\n<u>Пример:</u>\n<code>/poll Абу пидор? | Да | Естественно</code>\n<i>(2-5 вариантов)</i>",
                "<b>Глаза разуй, чучело!</b>\nРазделяй вопрос и ответы палочкой `|`.\n<u>Пример:</u>\n<code>/poll Я хуесос? | Да | Очень</code>\n<i>(2-5 вариантов)</i>",
                "<b>Ошибка формата, дегенерат!</b>\nСтавь `|` между вариантами.\n<u>Пример:</u>\n<code>/poll Аниме говно? | База | Соглы</code>\n<i>(2-5 вариантов)</i>"
            ]
            usage_text = random.choice(errs)
        try:
...
def get_help_keyboard(category: str, board_id: str, stream: str = 'ru') -> InlineKeyboardMarkup:
    ...
    if category == "main":
        ...
        if lang == 'en':
            builder.button(text="🛒 Market (/shop)", callback_data="shop_main_hub")
            builder.button(text="💼 Jobs (/work)", callback_data="work_main_hub")
            builder.button(text="🎰 Casino (/casino)", callback_data="casino_main_hub")
            builder.button(text="💳 Wallet (/wallet)", callback_data="prof_wallet")
            builder.button(text="📦 Lootboxes (/lootbox)", callback_data="shop_cat_lootbox")
            builder.button(text="🏆 Trophies (/ach)", callback_data="achievements_view")
        else:
            builder.button(text="🛒 В Магазин (/shop)", callback_data="shop_main_hub")
            builder.button(text="💼 На Работу (/work)", callback_data="work_main_hub")
            builder.button(text="🎰 В Казино (/casino)", callback_data="casino_main_hub")
            builder.button(text="💳 Мой Кошелек (/wallet)", callback_data="prof_wallet")
            builder.button(text="📦 Лутбоксы (/lootbox)", callback_data="shop_cat_lootbox")
            builder.button(text="🏆 Достижения (/ach)", callback_data="achievements_view")
...
async def periodic_board_summary():
    ...
    while True:
        ...
        try:
            ...
            logger.debug(f"📝 [PERIODIC SUMMARY] paragraphs={auto_paragraph_count} blat={is_blat} warhammer={is_warhammer} chunk_len={len(chunk)} chars")
            hf_token = os.getenv("HF_TOKEN")
            summary = await summarize_text_with_hf(prompt, chunk)
            ...
            if telegraph_url:
                if is_warhammer:
                    final_text = f"⚙️ <b>Вокс-депеша Механикус</b> ⚙️\n\nСвященный сервитор обработал инфо-потоки:\n🔗 <a href='{telegraph_url}'>Читать дата-свиток</a>"
                elif is_blat:
                ...
                if is_warhammer:
                    signet = "\n\n<i><b>⚙️ Запись заверена сервитором Адептус Механикус.</b>\nХвала Омниссии!</i>"
                    summary_truncated = _tg_safe_truncate(summary, max_utf16=3000)
                    final_text = f"⚙️ <b>Вокс-депеша /b/</b> ⚙️\n\n{summary_truncated}{signet}"
                elif is_blat:
                    signet = (
                        "\n\n<i><b>♠️ Малява составлена Смотрящим.</b>\n"
                        "Жизнь ворам, хуй мусорам! АУЕ!</i>"
...
async def drop_expiry_loop():
    """Фоновая очистка и возврат шекелей за невостребованные дропы."""
    while True:
        try:
...
async def setup_bot_commands(bots: dict):
    ...
    user_commands = [
        ...
        BotCommand(command="shit", description="Кинуть говно (реплай)"),
        BotCommand(command="rob", description="Ограбить анона (реплай)"),

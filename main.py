            ...
            try:
                ...
                if user_posts:
                    ...
                    for i in range(0, len(user_posts), chunk_size):
                        ...
                        query = "SELECT thread_id FROM Threads WHERE thread_id IN (SELECT CAST(value AS TEXT) FROM json_each(?)) OR thread_num IN (SELECT value FROM json_each(?))"
                ...
                if threads_to_delete:
                    t_json = json.dumps(threads_to_delete)
                    await db.execute("DELETE FROM Threads WHERE thread_id IN (SELECT CAST(value AS TEXT) FROM json_each(?))", (t_json,))

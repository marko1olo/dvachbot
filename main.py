...
class AnimeCommandHandler:
    """
    Handles parsing and processing of stacked anime commands,
    enforcing various limits (daily, hourly, spam) and assembling
    fetcher tasks and captions.
    """
    def __init__(self, message, board_id, stream='ru'):
        ...
        self.MAX_IMAGES = 10
        self.max_images_for_board = B_MAX_STACKED_ANIME_IMAGES if board_id == 'b' else self.MAX_IMAGES

        command_keys = '|'.join(ANIME_COMMAND_MAP.keys())
        self.pattern = re.compile(rf"/({command_keys})(?:(\d+)|(?:\s+(\d+)))?", re.IGNORECASE)
    ...
    async def execute(self):
        ...
        self._reset_hourly_tracker()
        self._calculate_requested_count()
        
        if await self._check_strict_daily_limit():
            return

        if await self._check_hourly_limit():
            return

        if await self._check_board_spam_limit():
            return

        if not self._prepare_fetcher_tasks():
            return
        ...
        await _process_stacked_anime_command(
            ...
            fetcher_tasks=self.fetcher_tasks,
            caption=caption,
            stream=self.stream
        )

    def _reset_hourly_tracker(self):
    ...
    def _calculate_requested_count(self):
        ...
        self.requested_count = min(raw_requested_count, self.max_images_for_board)
    ...
    async def _check_strict_daily_limit(self) -> bool:
        if self.user_id in self.b_data.get('anime_strict_limits', set()):
            ...
            if tracker['count'] + self.requested_count > 10:
                if self.lang == 'en':
                ...
                return True
            tracker['count'] += self.requested_count
        return False

    async def _check_hourly_limit(self) -> bool:
        if user_hourly_image_count[self.user_id] + self.requested_count > HOURLY_IMAGE_LIMIT:
        ...
        user_hourly_image_count[self.user_id] += self.requested_count
        return False
    ...
    def _prepare_fetcher_tasks(self) -> bool:
        canonical_map = {
        ...
        for command, num_no_space, num_with_space in self.matches:
            ...
            for _ in range(count):
                if len(self.fetcher_tasks) < self.max_images_for_board:
                    self.fetcher_tasks.append(cmd_func)
                    canonical_name = canonical_map.get(command_lower.split('@')[0])
                    if canonical_name:
                        self.command_counts[canonical_name] += 1
                else:
                    break
            if len(self.fetcher_tasks) >= self.max_images_for_board:
                break

        if not self.fetcher_tasks:
            return False

        if self.board_id == 'b':
            for _ in range(len(self.fetcher_tasks)):
    ...
    def _generate_caption(self) -> str:
        ...
        if not final_caption and random.random() < 0.30 and self.command_counts:
            population = list(self.command_counts.keys())
            weights = list(self.command_counts.values())
...
async def handle_stacked_anime_commands(message: types.Message, board_id: str | None, stream: str = 'ru'):
    ...
    handler = AnimeCommandHandler(message, board_id, stream)

...
async def _handle_missing_exception(update) -> bool:
    if update:
...
async def _handle_conflict_error(exception) -> bool:
    print(f"🌐 Конфликт: {exception}. Возможно, запущен другой экземпляр бота.")
    await asyncio.sleep(10)
    return True

async def _handle_bad_request_error(exception, update) -> bool:
...
async def _handle_unhandled_error(exception, update) -> bool:
    import traceback
...
async def global_error_handler(event: types.ErrorEvent) -> bool:
    ...
    if exception is None:
        return await _handle_missing_exception(update)
    if isinstance(exception, (TelegramNetworkError, aiohttp.ClientConnectorError, asyncio.TimeoutError, TelegramRetryAfter)):
        return await _handle_network_error(exception)
    if isinstance(exception, TelegramForbiddenError):
        return await _handle_forbidden_error(update)
    if isinstance(exception, TelegramConflictError):
        return await _handle_conflict_error(exception)
    if isinstance(exception, TelegramBadRequest):
        return await _handle_bad_request_error(exception, update)
    return await _handle_unhandled_error(exception, update)

...
async def _handle_telegram_forbidden_error(update, exception):
...
async def _handle_telegram_bad_request_error(update, exception):
...
async def _handle_unhandled_exception(update, exception):
    import traceback
...
async def global_error_handler(event: types.ErrorEvent) -> bool:
    ...
    if isinstance(exception, TelegramForbiddenError):
        return await _handle_telegram_forbidden_error(update, exception)
    ...
    if isinstance(exception, TelegramBadRequest):
        return await _handle_telegram_bad_request_error(update, exception)
        
    return await _handle_unhandled_exception(update, exception)

...
class BulkMessageSender:
    def __init__(self, bot_instance, board_id, recipients, content, reply_info, keyboard, verbose, queue_enqueued_at, queue_wait_sec, delivery_phase, delivery_original_recipients, delivery_deferred_recipients):
        ...
        self.stats = {'success': 0, 'ghosts': 0, 'errors': 0, 'blocks': 0, 'retries': 0, 'timeouts': 0, 'priority_recipients': 0, 'passive_recipients': 0}
    ...
    async def prepare(self):
        ...
        self.active_recipients = {uid for uid in self.recipients if uid > 0 and uid not in self.b_data['users']['banned']}
        if not self.active_recipients:
            return False

        self.original_recipients_count = self.delivery_original_recipients or len(self.active_recipients)
        self.ordered_recipients, self.stats['priority_recipients'], self.stats['passive_recipients'] = _order_recipients_for_delivery(
            self.board_id, self.active_recipients
        )
        ...
        if self.content.get('poll_data') and not self.final_keyboard:
            ...
            if poll_options and self.post_num:
                ...
                for i, option_text in enumerate(poll_options):
                    ...
                    buttons.append(InlineKeyboardButton(text=button_text, callback_data=f"poll_vote_{self.post_num}_{i}"))
    ...
    def _telegram_parse_error(self, err_low: str) -> bool:
        return (
    ...
    def _plain_delivery_text(self, head: str, body: str) -> str:
    ...
    def _log_plain_fallback(self, reason: str, current_content: dict) -> None:
    ...
    async def _send_plain_text_parts(
        self, uid: int, reason: str, current_content: dict, reply_to_mid: int | None,
        is_sage: bool, request_timeout: int, head: str, body: str,
        text: str | None = None, reply_to_id: int | None = None, include_keyboard: bool = True
    ):
        self._log_plain_fallback(reason, current_content)
        sent_msgs = []
        fallback_text = text if text is not None else self._plain_delivery_text(head, body)
    ...
    def _plain_media_source(self, current_content: dict, media_type: str):
    ...
    async def _send_plain_media_fallback(self, uid: int, reason: str, current_content: dict, full_text: str, reply_to_mid: int | None, is_sage: bool, has_spoiler: bool, request_timeout: int, head: str, body: str):
        plain_text = self._plain_delivery_text(head, body)
        ct = str(current_content.get("type") or "").split('.')[-1].lower()
        if ct == "text":
            return await self._send_plain_text_parts(uid, reason, current_content, reply_to_mid, is_sage, request_timeout, head, body, text=plain_text)
        if ct in ['photo', 'video', 'animation', 'document', 'audio', 'voice']:
            file_source = self._plain_media_source(current_content, ct)
            if not file_source:
                return await self._send_plain_text_parts(uid, reason, current_content, reply_to_mid, is_sage, request_timeout, head, body, text=plain_text)
            common_plain_kwargs = {
                'chat_id': uid,
            ...
            if len(plain_text) > 1024:
                ...
                await self._send_plain_text_parts(
                    uid, reason, current_content, reply_to_mid, is_sage, request_timeout, head, body,
                    text=plain_text, reply_to_id=media_msg.message_id, include_keyboard=False,
                )
                return media_msg
        ...
        if ct == "media_group":
            ...
            if not can_fit_caption:
                ...
                await self._send_plain_text_parts(
                    uid, reason, current_content, reply_to_mid, is_sage, request_timeout, head, body,
                    text=plain_text, reply_to_id=anchor_msg.message_id, include_keyboard=True,
                )
            self.stats['success'] += 1
            return res
        return await self._send_plain_text_parts(uid, reason, current_content, reply_to_mid, is_sage, request_timeout, head, body, text=plain_text)

    async def _send_text_fallback(self, uid: int, reason: str, full_text: str, current_content: dict, reply_to_mid: int | None, is_sage: bool, request_timeout: int):
        ...
        for i, part in enumerate(parts):
            m = await self.bot_instance.send_message(
                ...
                disable_notification=is_sage, disable_web_page_preview=True,
                request_timeout=request_timeout,
            )
            sent_msgs.append(m)
        self.stats['success'] += 1
            ...
            try:
                ...
                if ct == 'text':
                    ...
                    if self.media_url_text_fallback and current_content.get("image_url"):
                        return await self._send_text_fallback(uid, "cached_bad_media_url", full_text, current_content, reply_to_mid, is_sage, request_timeout)
                    if has_spoiler and ct in['photo', 'video', 'animation']:
                    ...
                    if len(full_text) > 1024:
                        ...
                        try:
                            ...
                            if self._telegram_parse_error(e.message.lower()):
                                await self._send_plain_text_parts(
                                    uid, "telegram_rejected_html_after_media", current_content, reply_to_mid,
                                    is_sage, request_timeout, head, body, text=self._plain_delivery_text(head, body),
                                    reply_to_id=media_msg.message_id, include_keyboard=False,
                                )
                                return media_msg
                            raise
                        self.stats['success'] += 1
                    ...
                    if not can_fit_caption:
                        ...
                        try:
                            ...
                            if self._telegram_parse_error(e.message.lower()):
                                await self._send_plain_text_parts(
                                    uid, "telegram_rejected_html_after_media_group", current_content, reply_to_mid,
                                    is_sage, request_timeout, head, body, text=self._plain_delivery_text(head, body),
                                    reply_to_id=anchor_msg.message_id, include_keyboard=True,
                                )
                                return res
                            raise
                ...
                if self._telegram_parse_error(err_low):
                    return await self._send_plain_media_fallback(uid, "telegram_rejected_html", current_content, full_text, reply_to_mid, is_sage, has_spoiler, request_timeout, head, body)
                if (
                    ...
                    return await self._send_text_fallback(uid, "telegram_rejected_media_url", full_text, current_content, reply_to_mid, is_sage, request_timeout)
    ...
    async def run(self):
        queue = deque(self.ordered_recipients)
        ...
        phase_deadline = self.start_time + phase_budget_sec if phase_budget_sec else None
    ...
    async def finalize(self, remaining_recipients_for_later, interrupted_reason, phase_budget_sec):
        ...
        if self.verbose:
            log_line = (
                ...
                f"✅ {self.stats['success']}/{len(self.active_recipients)} phase "
                f"({len(self.active_recipients)}/{self.original_recipients_count}, def {self.delivery_deferred_recipients}) | "
                f"🚫 {self.stats['blocks']} | "
                f"❌ {self.stats['errors']} | "
                f"👻 {self.stats['ghosts']} | "
                ...
                f"prio {self.stats['priority_recipients']}/{len(self.active_recipients)} | "
                f"⏱ {time_taken:.1f}s"
            )
            print(log_line)
            delivery_record = {
                ...
                "recipients": len(self.active_recipients),
                "phase_recipients": len(self.active_recipients),
                "original_recipients": self.original_recipients_count,
...
async def send_message_to_users(
    ...
    sender = BulkMessageSender(
        bot_instance, board_id, recipients, content, reply_info, keyboard,
        verbose, queue_enqueued_at, queue_wait_sec, delivery_phase,
        delivery_original_recipients, delivery_deferred_recipients
    )
    if not await sender.prepare():
        return []
    remaining_recipients, interrupted_reason, phase_budget_sec = await sender.run()
    return await sender.finalize(remaining_recipients, interrupted_reason, phase_budget_sec)

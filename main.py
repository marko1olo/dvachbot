...
async def cmd_global_unpin(message: types.Message, board_id: str | None, stream: str = 'ru'):
    ...
    if copies:
        ...
        await status_msg.edit_text(final)
async def _process_motivation_stream(board_id: str, stream: str, b_data: dict) -> None:
    if board_id == 'int':
        recipients = b_data['users']['active'] - b_data['users']['banned']
    else:
        if ENABLE_MULTILANG:
...
async def _process_board_motivation(board_id: str) -> None:
    activity = await get_board_activity_last_hours(board_id, hours=2)
    if activity < 20:
        return
    b_data = board_data[board_id]
    ...
    for stream in streams_to_process:
        await _process_motivation_stream(board_id, stream, b_data)
...
async def board_motivation_worker(board_id: str):
    while True:
        try:
            ...
            await asyncio.sleep(delay)
            await _process_board_motivation(board_id)
        except Exception as e:

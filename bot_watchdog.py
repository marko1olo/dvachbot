...
def _handle_child_exit(child: subprocess.Popen, return_code: int) -> bool:
    """Returns True if the supervisor should exit, False to restart."""
...
def _monitor_child(child: subprocess.Popen, start_time: float) -> bool:
    """Returns True if the supervisor should exit, False to restart."""
    health_failures = 0
    ...
    while True:
        ...
        if return_code is not None:
            return _handle_child_exit(child, return_code)
...
def main() -> int:
    ...
    while True:
        ...
        try:
            should_exit = _monitor_child(child, start_time)
            if should_exit:
                return 0

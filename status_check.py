...
async def get_queue_details(conn):
    ...
    async def fetch_queue(name, info):
        try:
            query = f"SELECT COUNT(*), MIN({info[1]}) FROM {info[0]} {f'WHERE {info[2]}' if len(info) > 2 else ''}"
            cursor = await conn.execute(query)
            count, oldest_ts = await cursor.fetchone()
            return name, {"count": count or 0, "oldest": oldest_ts or 0}
        except aiosqlite.OperationalError:
            return name, {"count": "N/A", "oldest": 0}

    tasks = [fetch_queue(name, info) for name, info in queue_map.items()]
...
async def get_activity(conn):
    ...
    for name, ts in periods.items():
        try:
            ...
            activity[f"posts_{name}"] = activity[f"threads_{name}"] = activity[
                f"users_{name}"
            ] = "N/A"
    return activity

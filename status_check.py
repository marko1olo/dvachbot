...
async def get_activity(conn):
    ...
    try:
        ...
        for name, ts in periods.items():
            query_parts.extend(
                [
                    "SELECT ? as key, COUNT(*) as val FROM Posts WHERE timestamp > ?",
                    "SELECT ? as key, COUNT(*) as val FROM Threads WHERE created_at > ?",
                    "SELECT ? as key, COUNT(DISTINCT user_id) as val FROM Users WHERE created_at > ?",
                ]
            )
            params.extend(
                [f"posts_{name}", ts, f"threads_{name}", ts, f"users_{name}", ts]
            )

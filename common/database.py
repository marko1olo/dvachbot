...
def _process_site_db_row(row):
    ...
    try:
        ...
        return None
async def _build_op_posts_query(
...
async def _fetch_op_posts_data(db, target_ids: List[int]):
    import json
...
async def _fetch_and_attach_replies(db, target_ids: List[int], posts_map: dict, real_thread_id_map: dict, viewer_id: int):
    import json
    id_placeholders = ','.join('?' for _ in target_ids)
    ...
    async with db_lock:
        for attempt in range(3):
            try:
                ...
                target_ids = await _build_op_posts_query(
                    board_id=board_id,
                    stream=stream,

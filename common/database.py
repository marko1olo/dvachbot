...
async def _create_indices(db):
    async with db.cursor() as cursor:
        ...
        await cursor.execute("CREATE INDEX IF NOT EXISTS idx_threadunlocks_thread_user ON ThreadUnlocks(thread_id, user_id);")
        await cursor.execute("CREATE INDEX IF NOT EXISTS idx_posts_shadow ON Posts(is_shadow);")

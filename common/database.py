    ...
    async with db_lock:
        for attempt in range(10):
            try:
                ...
                query = "SELECT post_num, content, is_shadow FROM Posts WHERE content LIKE ? ESCAPE '@'"
...
async def find_post_by_file_id(file_id_substring: str) -> dict | None:
    ...
    try:
        ...
        query = """
            ...
            LIMIT 1
        """
        escaped_file_id = str(file_id_substring).replace('@', '@@').replace('%', '@%').replace('_', '@_')
        search_pattern = '%' + escaped_file_id + '%'
...
async def get_posts_by_file_ids(file_ids: list[str]) -> list[dict]:
    ...
    for fid in file_ids:
        ...
        escaped_fid = str(fid).replace('@', '@@').replace('%', '@%').replace('_', '@_')
        params.append("%" + escaped_fid + "%")

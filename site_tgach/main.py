    ...
    while True:
        try:
            ...
            async with httpx.AsyncClient(transport=transport, timeout=30.0, verify=False) as client:
                ...
                if resp.status_code == 200:
                    new_nodes = set(re.findall(r'ExitAddress\s+(\d+\.\d+\.\d+\.\d+)', resp.text))
                    if new_nodes:
                        TOR_EXIT_NODES = new_nodes
                        logger.info(f"🛡️ TOR BLOCKER: Loaded {len(TOR_EXIT_NODES)} exit nodes.")
        except Exception as e:
...
def is_ip_restricted(ip_str: str) -> bool:
    ...
    try:
        ...
        if ip_str in TOR_EXIT_NODES: return True
        for subnet in BANNED_SUBNETS:
            if ip_obj in subnet: return True
    except: pass
    return False
...
SITE_CACHE_CLEANUP_INTERVAL_SEC = int(os.getenv("SITE_CACHE_CLEANUP_INTERVAL_SEC", "300"))
    ...
    while True:
        ...
        try:
            ...
            if len(BOT_VIOLATIONS) > 5000:
                logger.info(f"🧹 [Firewall] Pruning memory... (Tracking {len(BOT_VIOLATIONS)} IPs)")
...
async def lifespan(app: FastAPI):
    ...
    def startup_mark(label: str, started_at: float | None = None) -> float:
        ...
        if started_at is None:
            ...
            print(f"[site-startup] {label} ({(now - started_at) * 1000:.0f}ms)", flush=True)
        return now
...
def is_bot_by_headers(request: Request) -> bool:
    ...
    bot_uas = [
        ...
        "go-http-client", "libwww-perl", "python-urllib", "art-grabber"
    ]
...
async def ddos_guard_middleware(request: Request, call_next):
    ...
    if client_ip in IP_TROLL_CONFIG:
        ...
        if cfg['expires'] > time.time():
            ...
            m = cfg['mode']
            if m == 'gzip': 
                return StreamingResponse(managed_troll_generator(gzip_bomb_generator()), media_type="application/x-gzip", headers={"Content-Encoding": "gzip"})
            if m == 'html': 
                return StreamingResponse(managed_troll_generator(html_depth_charge_generator()), media_type="text/html")
            if m == 'xml': 
                return StreamingResponse(managed_troll_generator(xml_tarpit_generator()), media_type="application/xml")
            if m == 'js': 
                return StreamingResponse(managed_troll_generator(js_infinite_stream_generator()), media_type="application/javascript")
            if m == 'slow':
                return StreamingResponse(managed_troll_generator(slow_death_generator()), media_type="text/plain")
        else:
    ...
    if not path.startswith(("/static/", "/files/", "/favicon.ico")):
        ...
        if len(REQUEST_FLOOD_TRACKER[flood_key]) > limit_count:
            ...
            return StreamingResponse(
                ...
                headers={"Content-Encoding": "gzip", "Content-Disposition": f'attachment; filename="{filename}"'}
            )
...
async def sitemap_xml(request: Request):
    ...
    static_pages = [
        "", "/rules/", "/about/", "/faq/", 
        "/archive/threads/", "/archive/chat/", "/useful/", "/tags/"
    ]
    ...
    for page in static_pages:
        xml_content.append(f'  <url><loc>{base_url}{page}</loc><lastmod>{now_date}</lastmod><changefreq>daily</changefreq></url>')

    # 2. Добавляем индексы досок
    for board_id in BOARD_CONFIG:
        xml_content.append(f'  <url><loc>{base_url}/{board_id}/</loc><lastmod>{now_date}</lastmod><changefreq>always</changefreq></url>')
        xml_content.append(f'  <url><loc>{base_url}/{board_id}/catalog/</loc><lastmod>{now_date}</lastmod><changefreq>hourly</changefreq></url>')
    ...
    try:
        ...
        for tag_name, _ in popular_tags:
            ...
            xml_content.append(f'  <url><loc>{base_url}/tags/{safe_tag}</loc><lastmod>{now_date}</lastmod><changefreq>weekly</changefreq></url>')
    except Exception as e:
...
async def custom_404_handler(request: Request, exc):
    ...
    troll_patterns = [
        ...
        '/laravel', '/symfony', '/storage/logs', '/vendor', '/composer', '/ajax',
...
async def enrich_extra_data(posts: List[dict], is_ru: bool = True):
    ...
    for p in posts:
        ...
        if p.get('latest_replies'):
            for r in p['latest_replies']:
                apply_votes(r)
def _process_media_group_content(content: dict) -> None:
...
def _process_single_media_content(content: dict) -> None:
    if content.get('type') in {'photo', 'video', 'animation', 'document', 'audio', 'voice', 'sticker', 'video_note'} and 'files' not in content:
...
def _process_files_content(content: dict) -> None:
...
def _enrich_single_post(post: dict) -> None:
    if 'post_num' in post:
    ...
    _process_media_group_content(content)
    _process_single_media_content(content)
    _process_files_content(content)
...
def finalize_posts_for_user(posts: List[dict], user_id: int, is_ru: bool, op_author_id: str = None):
    """
...
async def overboard_page(request: Request, user: dict | None = Depends(get_optional_user)):
    ...
    if is_bot:
        ...
        if view_mode == 'threads':
            raw_posts = await get_op_posts_for_board(selected_boards, sort_by=sort_mode, page=1, page_size=50, stream=request.state.stream, observer_id=observer_id, ignore_pin=True, reply_limit=2)
        elif view_mode == 'posts':
            raw_posts = await get_global_feed_posts(selected_boards, page=1, page_size=50, stream=request.state.stream, observer_id=observer_id, include_chat=False, sort_by=sort_mode)
        else:
            raw_posts = await get_global_feed_posts(selected_boards, page=1, page_size=50, stream=request.state.stream, observer_id=observer_id, include_chat=True, sort_by=sort_mode)
    ...
    if tag_list:
        ...
        for post in posts:
            ...
            for f in files:
                ...
                if f.get('type') in ['image', 'photo', 'sticker', 'gif', 'video', 'animation', 'video_note']:
                    img_entry = f.copy()
...
@app.get("/tags/")
async def tags_index_page(request: Request, user: dict | None = Depends(get_optional_user)):
    ...
    if not user and backend:
        cached = await backend.get(cache_key)
        if cached: return HTMLResponse(content=cached)
...
async def tag_seo_page(request: Request, tag_name: str, page: int = 1, user: dict | None = Depends(get_optional_user)):
...
async def api_admin_troll_ip(data: AdminTrollRequest, user: dict = Depends(get_required_user)):
    ...
    log_system_event(f"🪤 TROLL ENABLED: {data.ip} | Mode: {data.mode} | For: {data.duration // 60} min")
...
async def read_thread(board_id: str, post_num: int, request: Request, user: dict | None = Depends(get_optional_user)):
    ...
    if is_bot:
        ...
        if user_id:
            ...
            if op_post.get('author_id') == user_hash: op_post['is_op_yours'] = True
            for reply in replies:
                if reply.get('author_id') == user_hash: reply['is_yours'] = True
    else:
        ...
        op_post = {
            ...
            'timestamp': time.time(),
            'author_id': 0,
            'is_pinned': False, 'is_endless': False, 'is_archived': False,
            'reply_count': 0, 'anon_count': 0
        }
        replies = []
...
async def read_board_catalog(board_id: str, request: Request, user: dict | None = Depends(get_optional_user)):
    ...
    if is_bot:
        observer_id = user['id'] if user else getattr(request.state, 'guest_id', 0)
        raw = await get_op_posts_for_board(board_id, sort_by=current_sort, page=1, page_size=100, stream=request.state.stream, observer_id=observer_id)
...
async def api_admin_toggle_blur(data: ToggleBlurRequest, request: Request, user: dict = Depends(get_required_user)):
    if not check_perm(user, 'mod'): raise HTTPException(status_code=403, detail="Нужен ранг Moderator")
    ...
    log_system_event(f"🌫️ CENSOR: Post #{data.post_num} censorship set to {new_state} by {user['id']}")
...
async def api_create_post(
    ...
    if is_shadow_muted:
        ...
        if sage: content['sage'] = True
        new_post_num = await create_post(
            author_id=int(user['id']), board_id=board_id, content=content,
            timestamp=time.time(), reply_to=reply_to, is_shadow_muted=True,
            is_from_site=True, post_mode=post_mode, stream=getattr(request.state, 'stream', 'ru')
        )
        return {"Status": "OK", "Num": new_post_num}
    t = request.state.t
    is_guest = user.get('is_guest', False)
    ...
    if board_id == 'wh40k':
        if text:
            _, res_content = await asyncio.to_thread(warhammer_transform, text, allow_image=False)
            text = res_content
...
async def cleanup_fastapi_cache_once() -> dict:
    ...
    async def _cleanup_unlocked():
        ...
        if SITE_FASTAPI_CACHE_MAX_KEYS > 0 and len(store) > SITE_FASTAPI_CACHE_MAX_KEYS:
            ...
            removable = sorted(
                store.items(),
                key=lambda item: getattr(item[1], "ttl_ts", 0)
            )[:overflow]
    ...
    if lock:
        ...
        await _cleanup_unlocked()
    return {"keys": len(store), "expired_removed": expired_removed, "cap_removed": cap_removed}
def cleanup_site_runtime_maps_once() -> dict:
    ...
    if SITE_THREAD_VERSION_TTL_SEC > 0:
        stale_thread_keys = [
            key for key, ts in list(THREAD_VERSIONS.items())
    ...
    if SITE_THREAD_VERSION_MAX_KEYS > 0 and len(THREAD_VERSIONS) > SITE_THREAD_VERSION_MAX_KEYS:
        overflow = len(THREAD_VERSIONS) - SITE_THREAD_VERSION_MAX_KEYS
        for key, _ in sorted(THREAD_VERSIONS.items(), key=lambda item: item[1])[:overflow]:
            THREAD_VERSIONS.pop(key, None)
            result["thread_versions_removed"] += 1
    if "URL_STATUS_CACHE" in globals():
        stale_urls = [
            key for key, value in list(URL_STATUS_CACHE.items())
            if isinstance(value, tuple) and len(value) > 1 and now - float(value[1] or 0) > 1800
        ]
    ...
    if SITE_FLOOD_TRACKER_TTL_SEC > 0:
        ...
        for key, history in list(REQUEST_FLOOD_TRACKER.items()):
            fresh_history = [ts for ts in history if now - float(ts or 0) < SITE_FLOOD_TRACKER_TTL_SEC]
            if fresh_history:
    ...
    if SITE_SECURITY_MAP_MAX_KEYS > 0 and len(REQUEST_FLOOD_TRACKER) > SITE_SECURITY_MAP_MAX_KEYS:
        ...
        for key, _ in sorted(
            ...
            REQUEST_FLOOD_TRACKER.pop(key, None)
            result["flood_tracker_removed"] += 1
    expired_bans = [ip for ip, exp in list(IP_BAN_LIST.items()) if float(exp or 0) < now]
    for ip in expired_bans:
        IP_BAN_LIST.pop(ip, None)
        result["ip_bans_removed"] += 1
    expired_trolls = [
        ip for ip, cfg in list(IP_TROLL_CONFIG.items())
    ...
    if SITE_SECURITY_MAP_MAX_KEYS > 0 and len(BOT_VIOLATIONS) > SITE_SECURITY_MAP_MAX_KEYS:
    ...
    while True:
        try:
            ...
            if (
                ...
                logger.info(f"Site cache cleanup: cache={cache_result}, maps={map_result}")
        except asyncio.CancelledError:
            raise
        except Exception as exc:
...
def get_site_process_snapshot() -> dict:
    ...
    try:
        ...
        try:
            ...
            full_info = info
        private_bytes = getattr(full_info, "uss", None) or getattr(info, "private", None) or getattr(info, "vms", 0)
        return {
            "pid": os.getpid(),
...
def get_site_runtime_snapshot() -> dict:
    ...
    return {
        ...
        "url_status_cache": len(URL_STATUS_CACHE) if "URL_STATUS_CACHE" in globals() else 0,
        "request_flood_tracker": len(REQUEST_FLOOD_TRACKER),
        "known_ips": len(KNOWN_IPS),
...
async def api_get_catalog(board_id: str, request: Request, sort: str = "bump", user: dict = Depends(get_current_user_or_guest)):
    board_id = board_id.lower()
    if board_id not in BOARD_CONFIG: raise HTTPException(404)
    current_version = BOARD_VERSIONS[board_id]
    stream = getattr(request.state, 'stream', 'ru')
    ...
    if cached:
        ...
        raw = await get_op_posts_for_board(board_id, sort_by=sort, page=1, page_size=100, stream=stream)
        posts = _convert_and_enrich_posts(raw)
        await enrich_heavy_data(posts)
        if posts: await backend.set(cache_key, orjson.dumps(posts), expire=600)
    import copy
...
async def api_get_threads(
    ...
    current_version = BOARD_VERSIONS[board_id] if board_id in BOARD_VERSIONS else time.time()
    stream = getattr(request.state, 'stream', 'ru')
    view_mode = request.query_params.get("view", "threads")
    ...
    if not posts_container:
        ...
        if board_id == "overboard" and view_mode in ['posts', 'all']:
            ...
            posts_container = await asyncio.to_thread(_convert_and_enrich_posts, raw_posts)
        else:
            # Режим списка тредов
            op_posts = await get_op_posts_for_board(
                target_board, 
            ...
            posts_container = await asyncio.to_thread(_convert_and_enrich_posts, op_posts)
...
async def api_admin_delete_post(data: AdminAction, request: Request, user: dict = Depends(get_required_user)):
    ...
    if await delete_post_by_num(data.post_num):
        ...
        for s in ['ru', 'en', 'jp']:
            await FastAPICache.clear(namespace="main", key=f"thread_html:{bid}:{tid}:{s}")
...
async def _fetch_telegram_path(file_id: str, bot_token: str):
    ...
    if GLOBAL_HTTP_SESSION is None or GLOBAL_HTTP_SESSION.closed:
        ...
        connector = aiohttp.TCPConnector(limit=250, ttl_dns_cache=300, family=socket.AF_INET)
...
async def get_telegram_file(file_id: str, request: Request, filename: str = None):
    ...
    is_video_ext = any(x in request.url.path.lower() for x in ['mp4', 'webm', 'mov', 'gif'])
    ...
    for attempt in range(max_attempts): 
        ...
        if attempt == 0 and file_id.startswith('AgAC'):
             cached_path_info = await get_cached_file_path(file_id)
             if cached_path_info: break # Путь есть, можно отдавать

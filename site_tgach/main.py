...
async def check_and_punish_site_spam(
    ...
    if files:
        ...
        async def _hash_file(img):
            try:
                await img.seek(0)
                content = await img.read()
                await img.seek(0)
                if content:
                    return await asyncio.to_thread(lambda c: hashlib.sha256(c).hexdigest(), content)
            except Exception:
                import traceback; traceback.print_exc()
            return None
        ...
        file_hashes = []
        if files:
            results = await asyncio.gather(*(_hash_file(img) for img in files))
            file_hashes = [h for h in results if h is not None]
...
async def shadow_cleanup_task():
    while True:
        ...
        try:
            ...
            logger.error(f"⚠️ Maintenance task error: {e}", exc_info=True)
...
async def sitemap_xml(request: Request):
    ...
    try:
        ...
        logger.error(f"Sitemap threads error: {e}", exc_info=True)
    ...
    try:
        ...
        logger.error(f"Sitemap tags error: {e}", exc_info=True)
...
async def mtproto_cleanup_task():
    while True:
        ...
        try:
            ...
            logger.error(f"MTProto cleanup error: {e}", exc_info=True)
...
async def api_admin_ban_image(
    ...
    try:
        ...
        if fids:
            import json
            async with db.execute(
                "SELECT sha256, phash FROM FileRegistry WHERE file_id IN (SELECT value FROM json_each(?))",
                (json.dumps(fids),)
            ) as cursor:
                rows = await cursor.fetchall()
                for row in rows:

            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if resp.status_code == 400 and "refusal" in resp.text.lower():
                        return {"visual_style": "photorealistic", "subject_age_visual": "child", "safety_flags": ["REFUSAL_HARD_CSAM"]}
...
async def run_deep_check(image_bytes: bytes, file_id: str):
    ...
    is_hard_violation = (visual_style == 'photorealistic' and age in ['child', 'toddler', 'infant']) or ('REFUSAL_HARD_CSAM' in flags)
    ...
    if is_hard_violation:
        ...
        for pid in affected_posts:
            ...
            if post:
                ...
                ip_addr = post.get('ip') # Берем IP из записи поста (если реализовано в схеме)
                ...
                if uid and uid not in banned_authors:
                    ...
                    if ip_addr:
                        ...
                        # ПРАВКА: Записываем бан в БД, чтобы основной сервер подхватил его
                        try:
                            # Добавляем IP в список глобального фаервола в БД
                            current_fw = await get_pool()
                            async with current_fw.execute("SELECT value FROM SystemSettings WHERE key = 'ip_blacklist'") as c:
                                row = await c.fetchone()
                                blacklist = row[0] if row and row[0] else ""
                            
                            if ip_addr not in blacklist:
                                new_blacklist = f"{blacklist},{ip_addr}".strip(',')
                                await set_system_setting('ip_blacklist', new_blacklist)
                        except Exception as fw_err:
                            logger.error(f"Failed to sync IP ban to DB: {fw_err}")
    ...
    # === ЛОГИКА МЯГКОЙ ЦЕНЗУРЫ (ПОДРОСТКИ / BORDERLINE) ===
        ...
        logger.warning(f"🌫️ [DeepCheck] Applied BLUR to file {file_id}. Reason: {reason}")

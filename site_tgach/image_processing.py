...
techniques such as applying filters, generating thumbnails, and calculating perceptual hashes 
for deduplication purposes.
Key Functions:
- clean_tags_string: Cleans and formats a string by removing extra spaces and commas.
- apply_srgb_to_linear: Converts an sRGB value to linear color space.
- sign_pow: Computes the signed power of a value.
- encode_83: Encodes a value into a base-83 string.
- encode_dc: Encodes the DC component of a blurhash.
- encode_ac: Encodes the AC components of a blurhash.
- encode_blurhash_internal: Generates a blurhash string from an image.
- _grimdark_worker: Processes an image to apply a "grimdark" filter in a separate process.
- apply_grimdark_filter_async: Asynchronous wrapper for the grimdark filter worker.
- process_and_upload_image: Processes an uploaded file, checks for deduplication, 
    generates thumbnails, and uploads the file to the specified Telegram channel.
Constants:
- MIRROR_SEMAPHORE: Limits the number of concurrent uploads.
- THUMBNAIL_SIZE: Defines the size of generated thumbnails.
- TELEGRAM_PHOTO_LIMIT: Maximum size for photos uploaded to Telegram.
- TELEGRAM_DOCUMENT_LIMIT: Maximum size for documents uploaded to Telegram.
Dependencies:
...
- FastAPI: For handling file uploads and HTTP exceptions.
- PIL (Pillow): For image processing.
- aiogram: For interacting with the Telegram Bot API.
- common.database: For database operations related to file handling.
- site_tgach.catbox: For uploading files to Catbox.
- site_tgach.huggingface: For uploading files to Hugging Face.
- site_tgach.mtproto_client: For MTProto file uploads.
- common.bot_pool: For managing a pool of Telegram bots.
"""
import logging
import re
import time
        ...
        async def _send_with_retry(method_name, input_file_obj, **kwargs):
            ...
            for attempt in range(1, max_attempts + 1): 
                try:
                    ...
                    return await method(request_timeout=35, **kwargs)
                except TelegramRetryAfter as e:
                    if e.retry_after > 10: raise # Если флудвейт большой, не ждем

...
"""
importer.py
This module contains the ThreadImporter class, which is responsible for importing threads and posts from various imageboard platforms.
It handles the downloading of media files, normalizes HTML content, and interacts with a database to store posts and threads.
Classes:
    - MemoryUploadFile: A wrapper class for BytesIO that simulates the behavior of UploadFile from FastAPI.
    - ThreadImporter: A class that manages the import of threads and posts, including media handling and database interactions.
Methods:
    - _cleanup: Rolls back transactions by deleting created posts in case of an error during insertion.
    - _normalize_html_sync: Synchronously normalizes raw HTML content by removing unwanted tags and replacing specific terms.
    - normalize_html: Asynchronously normalizes HTML content using a thread pool executor.
    - _safe_request: Safely makes an HTTP request with retries for handling transient errors.
    - fetch_json: Fetches JSON data from a given URL, handling various imageboard-specific URL formats.
    - extract_posts_data: Extracts post data from the fetched JSON response.
    - _process_single_post_media: Downloads and processes media files associated with a single post, handling various URL formats and ensuring proper uploads.
Constants:
    - PROXY_URL: The URL for the proxy server used for HTTP requests.
    - RE_LINK_REF: A regular expression pattern for matching link references in posts.
    """
import httpx
import re
import random
...
class ThreadImporter:
    ...
    async def normalize_html(self, raw_html: str) -> str:
        ...
        return await loop.run_in_executor(
            self._cpu_executor, self._normalize_html_sync, raw_html
        )
    ...
    async def _safe_request(
        ...
        for i in range(retries):
            try:
                # Используем временный клиент для каждого файла, чтобы избежать проблем с пулом соединений (RemoteProtocolError)
                # Это чуть медленнее, но гораздо надежнее для 2ch/4chan
                req_headers = {"User-Agent": self.client.headers.get("user-agent", "")}

                # Передаем куки только если это 2ch (для доступа к скрытым разделам, если есть пасскод)
                req_cookies = self.client.cookies if "2ch" in url else {}
                ...
                # Логируем как warning, так как будет ретрай
                logger.warning(
                    f"⚠️ Network error downloading {url} (Attempt {i+1}): {e}"
                )
        ...
        if post.get("extra_files"):
            for ef in post["extra_files"]:
                if ef.get("tim") and ef.get("ext"):
                    ...
                    display_name = (ef.get("filename") or str(ef["tim"])) + str(
                        ef["ext"]
                    )
                    files_to_download.append({"path": filename, "name": display_name})
                ...
                for media_url in urls_to_try:
                    try:
                        ...
                        if resp and resp.status_code == 200:
                            ...
                            if "text/html" in ct:
                                logger.warning(
                                    f"⚠️ Got HTML instead of image from {media_url}"
                                )
                                continue
                        ...
                        if (
                            ...
                            processed_bytes = await apply_grimdark_filter_async(
                                raw_bytes
                            )
        ...
        def replace_ref(match):
            ...
            if old_ref in id_map:
                ...
                if reply_to_id is None:
                    reply_to_id = new_id
                return f">>{new_id}"
            # Если ссылки нет в карте переназначения, возвращаем её как есть, а не пустую строку
    ...
    async def process_thread(
        ...
        logger.info(
            f"🚀 START IMPORT: {source_url} -> /{target_board}/ [{stream}] Sim: {use_sim}"
        )
        ...
        if (
            "4chan.org" in source_url
            or "4channel.org" in source_url
            or "4cdn.org" in source_url
        ):
        ...
        try:
            ...
            if os.path.exists(cookie_path):
                ...
                logger.info(
                    f"🍪 Loaded cookies from {cookie_filename}: {list(cookies.keys())}"
                )
            else:
        ...
        async with httpx.AsyncClient(
            ...
            try:
                ...
                try:
                    ...
                    logger.error(
                        f"❌ Phase 1 FAIL: Failed to fetch/parse thread JSON: {e}",
                        exc_info=True,
                    )
                    return
                ...
                personas = [
                    int(random.randint(-9999999, -1000000))
                    for _ in range(unique_personas_count)
                ]
                ...
                is_4chan = (
                    "4chan" in source_url
                    or "4channel" in source_url
                    or "4cdn" in source_url
                )
                ...
                for i, p in enumerate(posts):
                    ...
                    try:
                        ...
                        raw_text = (
                            p.get("comment") or p.get("com") or p.get("body") or ""
                        )
                        raw_text = raw_text.replace("<wbr>", "")
                        clean_text = await self.normalize_html(raw_text)

                        if target_board == "wh40k":
                            _, w_content = warhammer_transform(
                                clean_text, allow_image=False
                            )
                        ...
                        files_data = await self._process_single_post_media(
                            p,
                            base_url,
                            domain_root,
                            semaphore,
                        ...
                        logger.error(
                            f"⚠️ Error processing post {i} (skipped): {post_error}"
                        )
                        continue
                ...
                if use_sim:
                    await self._queue_posts_for_simulation(
                        prepared_posts, target_board, stream, task_id, sim_settings
                    )
                else:
    ...
    async def _queue_posts_for_simulation(
        self,
        posts: List[Dict],
        board_id: str,
        stream: str,
    ...
    async def _instant_import(
        self, prepared_posts: List[Dict], target_board: str, stream: str
    ):
        id_map: Dict[str, int] = {}
        new_thread_id = None
        all_files_for_queue = []
        ...
        async with db_lock, get_db_connection() as conn:
            try:
                ...
                title = (
                    raw_subject
                    if raw_subject
                    else (op_data["text"][:100] if op_data["text"] else "Thread")
                )
                await conn.execute(
                    """INSERT INTO Threads 
...
=======
                    ...
                    for p_data in chunk:
                        ...
                        params.extend([target_board, new_thread_id, post_content, p_data["timestamp"], p_data["author_id"], stream])
                    ...
                    for p_data, row in zip(chunk, new_ids):
                        new_id = row[0]
                        self.created_post_ids.append(new_id)
                        id_map[p_data["old_id"]] = new_id
>>>>>>> 1d0451e (⚡ Optimize importer chunk loop to use single parameterized insert)
                    await conn.commit()

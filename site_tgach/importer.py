...
class ThreadImporter:
        ...
        for i in range(retries):
            try:
                ...
                async with httpx.AsyncClient(
                    ...
                    proxy=PROXY_URL,
                    cookies=req_cookies,
                    transport=httpx.AsyncHTTPTransport(local_address="0.0.0.0" if os.getenv("FORCE_IPV4", "1").lower() in ("1", "true", "yes") else None, retries=2)
                ) as media_client:
                ...
                logger.warning(f"⚠️ Network error downloading {url} (Attempt {i+1}): {e}")
    ...
    async def process_thread(self, source_url: str, target_board: str, stream: str = 'ru', sim_settings: dict = None):
        ...
        transport = httpx.AsyncHTTPTransport(
            local_address="0.0.0.0" if os.getenv("FORCE_IPV4", "1").lower() in ("1", "true", "yes") else None, # Принудительный IPv4 (Fix для OpenVPN)
            retries=3,
            verify=False,
    ...
    async def _instant_import(self, prepared_posts: List[Dict], target_board: str, stream: str):
        ...
        async with db_lock, get_db_connection() as conn:
            try:
                ...
                title = raw_subject if raw_subject else (op_data["text"][:100] if op_data["text"] else "Thread")
                await conn.execute(
                    """INSERT INTO Threads 

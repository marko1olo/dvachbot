...
BOT_CLAIM_EXCUSES: List[str] = [
    ...
    "🛑 Клик бота за {delta:.2f}с аннулирован! Настоящий сыч сначала 2 секунды чешет пузо.",
    "🤖 Скриптовая макака поймана с поличным ({delta:.2f}с). Выпей машинного масла и остынь.",
    "⚡ Слишком быстро ({delta:.2f}с)! Даже нейроны в твоем скудном мозгу бегают медленнее. Чек защищен.",
    ...
    "🤖 Ошибка биометрии: скорость робота {delta:.2f}с характерна для стиральной машины, а не двачера.",
    "⚡ Реакция {delta:.2f}с? Ты чё, спидами обкололся? Защита от читеров заблокировала перехват.",
    "🤖 Твой питоновский скрипт на requests/aiohttp обосрался на скорости {delta:.2f}с. Гуляй.",
    "🛑 Клик автокликера за {delta:.2f}с заблокирован анти-турбо защитой Абу. Ручками тыкай!",
...
CLAIM_QUOTA_EXCUSES: List[str] = [
    ...
    "Абу наложил вето на твою жадность. Лимит исчерпан: 3 дропа за 5 минут собрано — иди переваривай.",
...
ACTIVE_DROP_EXCUSES: List[str] = [
    ...
    "Твой прошлый чек шекелей всё ещё висит в треде. Второй создавать нельзя.",
    ...
    "Твой предыдущий дроп ещё не нашёл своего хозяина. Жди окончания таймера или сбора.",
...
BOARD_FLOOD_EXCUSES: List[str] = [
    ...
    "Шекелемет доски перегружен дропами! Остывание ствола займет ещё {seconds}с.",
    ...
    "Слишком частый спам раздачами ломает базу данных! Таймаут доски: {seconds}с.",
    ...
    "Двачане не успевают за потоком дропов! Сбавьте темп на {seconds}с.",
...
async def create_money_drop(
    ...
    fee_percent: float = DEFAULT_DROP_FEE_PERCENT,
    ...
    async with db_lock:
        try:
            ...
            if fee > 0:
                await record_user_transaction(db_conn, donor_id, -amount, 'drop', f'Сброс чека в тред (#{drop_id}), комиссия {int(fee_percent * 100)}%: {fee} ₪')
                await add_to_abu_fund(db_conn, fee)
            else:

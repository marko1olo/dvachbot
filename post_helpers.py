...
def _get_random_header_prefix(lang: str = 'ru') -> str:
    ...
    if rand_prefix < 0.070: return "Claude Fable 5.1 - "
    ...
    if rand_prefix < 0.154: return "Безногий высиран ZVO - "
    if rand_prefix < 0.156: return "Мыкола из ТЦК - "
    if rand_prefix < 0.158: return "Оператор - "
    if rand_prefix < 0.160: return "### ОБОССАННЫЙ ### "
    if rand_prefix < 0.162: return "### БИОМУСОР ### "
    if rand_prefix < 0.164: return "Лоли - "

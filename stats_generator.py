...
def generate_all_charts():
    ...
    )  # 60 days to show weekly trends better
    data = c.fetchall()
    if data:
        df = pd.DataFrame(data)
        fig, ax = plt.subplots(figsize=(10, 5))
        ...
        plt.title(
            "2. Размер онлайна (Уникальные шизы за НЕДЕЛЮ)",
            fontsize=16,
            fontweight="bold",
            color="#00ffcc",
        )
    ...
    if data:
        ...
        for d, stats in sorted(daily_stats.items()):
            toxic_percent = (
                (stats["toxic"] / stats["total"]) * 100 if stats["total"] > 0 else 0
            )
        ...
        ax.set_xticklabels(
            df["d"].tolist()[::step3], rotation=45, ha="right", fontsize=7
        )
        plt.title(
            "3. Матоемкость (% постов с матами)",
            fontsize=16,
            fontweight="bold",
            color="#ff0000",
        )
    ...
    if data:
        ...
        plt.title(
            "4. Топ-10 Главных Шизоидов (По количеству высеров)",
            fontsize=16,
            fontweight="bold",
            color="#ff9900",
        )
    ...
    if data:
        ...
        plt.suptitle(
            "5. Главные Байтеры — Топ-20 (Кому больше всего реплаят)",
            fontsize=15,
            fontweight="bold",
            color="#33ccff",
            y=1.01,
    ...
    if row789:
        ...
        _donuts = [
            {
                "ax": axes[0],
                "sizes": [row789["night_posts"] or 0, row789["day_posts"] or 0],
                "labels": ["Ночь\n(01-06)", "День"],
                "colors": ["#6600cc", "#ffcc00"],
                "title": "7. Клуб\nПолуночников",
                "tc": "#aa88ff",
            },
            {
                "ax": axes[1],
                "sizes": [row789["media_posts"] or 0, row789["text_posts"] or 0],
                "labels": ["Медиа", "Текст"],
                "colors": ["#ff3399", "#cccccc"],
                "title": "8. Картинко-\nдрочеры",
                "tc": "#ff3399",
            },
            {
                "ax": axes[2],
                "sizes": [row789["replies"] or 0, row789["singles"] or 0],
                "labels": ["Диалог", "Монолог"],
                "colors": ["#00ff99", "#555555"],
                "title": "9. Уровень\nДискуссии",
                "tc": "#00ff99",
            },
        ]
        for _d in _donuts:
            ...
            _ws, _ts, _ats = _ax.pie(
                ...
                startangle=90,
                colors=_d["colors"],
                wedgeprops=dict(width=0.55, edgecolor="#121212", linewidth=2),
                pctdistance=0.75,
            )
            for _at in _ats:
                _at.set_fontsize(11)
                _at.set_fontweight("bold")
                _at.set_color("#ffffff")
            for _t in _ts:
                _t.set_color("#dddddd")
                _t.set_fontsize(9)
            _ax.set_title(
                _d["title"], fontsize=13, fontweight="bold", color=_d["tc"], pad=12
            )
        plt.suptitle(
            "7–9. Профиль Анона: время / формат / диалог (30д)",
            fontsize=14,
            fontweight="bold",
            color="#ffffff",
            y=1.02,
        ...
        plt.savefig(buf, format="png", bbox_inches="tight")
        buf.seek(0)
        images.append(("7_8_9_donut_panel.png", buf))
        plt.close()
    ...
    if data:
        ...
        sns.heatmap(heatmap_data, cmap="inferno", linewidths=0.5, ax=ax)
        plt.title(
            "10. Циркадные ритмы Анона (Активность по часам/дням)",
            fontsize=16,
            fontweight="bold",
            color="#ffaa00",
        )
        ...
        if edges_data:
            ...
            if len(G) > 0:
                ...
                nx.draw_networkx_edges(
                    G_sub, pos, alpha=0.3, edge_color="#555555", ax=ax
                )

                plt.title(
                    "11. Граф Социального Пузыря (Эхо-камеры)",
                    fontsize=16,
                    fontweight="bold",
                    color="#00ffcc",
                )
    ...
    try:
        if edges_data:
            ...
            if len(DiG) > 0:
                ...
                plt.title(
                    "12. Топ-10 Хабов Внимания (PageRank)",
                    fontsize=16,
                    fontweight="bold",
                    color="#ff00ff",
                )
                ...
                for idx, row in df_pr.iterrows():
                    ax.text(
                        row["pagerank"] + (ax.get_xlim()[1] * 0.01),
                        idx,
                        f"{row['pagerank']:.4f}",
                        va="center",
                        ha="left",
    ...
    try:
        if edges_data:
            ...
            if mutual_list:
                sorted_mutual = sorted(mutual_list, key=lambda x: x[2], reverse=True)[
                    :5
                ]
                plot_data = []
                ...
                plt.title(
                    "13. Топ-5 Взаимных Перепихонов (Circlejerk)",
                    fontsize=16,
                    fontweight="bold",
                    color="#00ff66",
                )
    ...
    try:
        ...
        if posts_timeline:
            ...
            if session_durations:
                ...
                fig, ax = plt.subplots(figsize=(10, 5))
                sns.histplot(
                    df_sess["duration"], bins=30, color="#ff9933", kde=True, ax=ax
                )
                plt.title(
                    "14. Длина непрерывного залипания (Сессии)",
                    fontsize=16,
                    fontweight="bold",
                    color="#ff9933",
                )
    ...
    try:
        ...
        c.execute(
            "SELECT content, author_id FROM Posts WHERE timestamp > ?",
            (seven_days_ago,),
        )
        this_week_posts = c.fetchall()

        c.execute(
            "SELECT content, author_id FROM Posts WHERE timestamp BETWEEN ? AND ?",
        ...
        def get_word_counts(posts_list):
            ...
            for row in posts_list:
                ...
                if "[INFO]" in text or "[DEBUG]" in text or "[ERROR]" in text:
                    continue
                if (
                    "ChatGPT Cringe Report" in text
                    or "нелепых и шаблонных фразах" in text
                ):
                    continue
        ...
        for word, c1 in this_week_counts.items():
            ...
            if c1 >= 5 and c1 > c2:
                ...
                rising_words.append(
                    {"word": word, "c1": c1, "c2": c2, "change": pct_change}
                )

        rising_words = sorted(rising_words, key=lambda x: x["change"], reverse=True)[
            :20
        ]
        ...
        if rising_words:
            ...
            if not df_left.empty:
                ...
                for idx, row in df_left.iterrows():
                    ax1.text(
                        row["change"] + (ax1.get_xlim()[1] * 0.01),
                        idx,
                        f"+{int(row['change'])}%",
                        va="center",
                        fontsize=9,
            ...
            if not df_right.empty:
                ...
                for idx, row in df_right.iterrows():
                    ax2.text(
                        row["change"] + (ax2.get_xlim()[1] * 0.01),
                        idx,
                        f"+{int(row['change'])}%",
                        va="center",
                        fontsize=9,
            ...
            plt.suptitle(
                "15. Мем-Радар: Взлетающие Тренды (Прирост за неделю)",
                fontsize=16,
                fontweight="bold",
                color="#ffaa00",
                y=0.96,
    ...
    try:
        ...
        if word_data:
            ...
            if top_words:
                ...
                plt.title(
                    "16. Частотный Шитпост-Словарь (Топ-30 Слов)",
                    fontsize=16,
                    fontweight="bold",
                    color="#ff33cc",
                )
                ...
                for idx, row in df_words.iterrows():
                    ax.text(
                        row["Частота"] + (ax.get_xlim()[1] * 0.01),
                        idx,
                        f"{int(row['Частота'])}",
                        va="center",
                        ha="left",
        ...
        if sentiment_posts:
            ...
            for r in sentiment_posts:
                ...
                try:
                    content_dict = json.loads(r["content"])
                    text = (
                        content_dict.get("text") or content_dict.get("caption") or ""
                    ).lower()
                except:
            ...
            if plot_data:
                ...
                ax.plot(
                    xs17, vals17, marker="o", color="#aaaaaa", linewidth=1.5, zorder=3
                )
                ax.fill_between(
                    xs17,
                    vals17,
                    0,
                    where=[v >= 0 for v in vals17],
                    color="#33cc66",
                    alpha=0.3,
                    label="База",
                )
                ...
                ax.set_xticklabels(
                    df_sent["d"].tolist()[::step17], rotation=45, ha="right", fontsize=7
                )
                ax.legend(fontsize=9)
                plt.title(
                    "17. Индекс Токсичности (Двачевский сентимент)",
                    fontsize=16,
                    fontweight="bold",
                    color="#ff3333",
                )
    ...
    try:
        ...
        if ttr_data:
            ...
            sorted_users = sorted(
                user_texts.items(), key=lambda x: len(x[1]), reverse=True
            )[:10]
            ...
            for uid, texts in sorted_users:
                ...
                segments = [
                    ...
                    if len(words[i : i + segment_size]) == segment_size
                ]
            ...
            if msttr_results:
                ...
                plt.title(
                    "18. Лексическое Разнообразие (Разнообразие Словарного Запаса)",
                    fontsize=16,
                    fontweight="bold",
                    color="#ffcc00",
                )
                plt.xlabel(
                    "Индекс разнообразия слов (выше = богатый язык, ниже = спамер 3 фраз)"
                )
                plt.ylabel("")
                ax.set_xlim(0, df_ttr["msttr"].max() * 1.15)
                for idx, row in df_ttr.iterrows():
                    ax.text(
                        row["msttr"] + (ax.get_xlim()[1] * 0.01),
                        idx,
                        f"{row['msttr']:.3f}",
                        va="center",
                        ha="left",
    ...
    try:
        c.execute(
            "SELECT date(timestamp, 'unixepoch', 'localtime') as d, content FROM Posts WHERE timestamp > ?",
            (thirty_days_ago,),
        )
        ...
        if plot_rows:
            ...
            df_counts = (
                df_reading.groupby(["d", "category"]).size().reset_index(name="cnt")
            )
            pivot_df = df_counts.pivot(
                index="d", columns="category", values="cnt"
            ).fillna(0)
    ...
    try:
        ...
        if data:
            ...
            HEAT = LinearSegmentedColormap.from_list(
                "dv", ["#0d1117", "#003d20", "#006d35", "#39d353", "#80ffaa"]
            )
            im = ax.imshow(grid, cmap=HEAT, aspect="auto", interpolation="nearest")

            ax.set_xticks(range(24))
            ax.set_xticklabels(
                [f"{h:02d}:00" for h in range(24)], fontsize=7, rotation=45, ha="right"
            )
            ax.set_yticks(range(7))
            ax.set_yticklabels(days_ru_full, fontsize=8)
            plt.title(
                "21. Тепловая карта час × день недели (180д)",
                fontsize=15,
                fontweight="bold",
                color="#ffffff",
            )
        ...
        if data:
            ...
            for idx, d in enumerate(range(6, -1, -1)):
                ...
                ax2.text(
                    23.4,
                    0.40,
                    f'{total_d//1000 if total_d>=1000 else total_d}{"k" if total_d>=1000 else ""}',
                    ha="left",
                    va="center",
                    color=color,
                    fontsize=7.5,
    ...
    try:
        ...
        if data:
            ...
            ax.bar(
                ...
                width=width,
                bottom=0.12,
                color=[cmap(v) for v in vals_norm],
                alpha=0.92,
                edgecolor="#121212",
                linewidth=0.7,
            )
            ...
            ax.grid(False)
            ax.set_title(
                f"23. Часовой циферблат активности (90д)\n"
    ...
    try:
        ...
        if data:
            ...
            cb = fig.colorbar(
                im, ax=ax, orientation="horizontal", pad=0.18, shrink=0.35
            )
    ...
    try:
        ...
        if data:
            ...
            ax.set_xticklabels(
                [df["d"].iloc[i] for i in range(0, len(df), step)],
                rotation=30,
                fontsize=7.5,
            )
            ax.set_title(
            ...
            plt.savefig(buf, format="png")
            buf.seek(0)
            images.append(("25_cumulative.png", buf))
            plt.close()
    except Exception as e:
    ...
    try:
        ...
        if data:
            ...
            bars = ax.bar(
                labels, vals, color=colors, edgecolor="#21262d", linewidth=1.2
            )
            ...
            plt.savefig(buf, format="png")
            buf.seek(0)
            images.append(("26_reply_depth.png", buf))
            plt.close()
    except Exception as e:
    ...
    try:
        ...
        c2.execute(
            "SELECT COUNT(DISTINCT author_id) as n FROM Posts WHERE timestamp > ?",
            (t30,),
        )
        uniq30 = c2.fetchone()["n"]
        c2.execute(
            "SELECT COUNT(*) as n FROM Posts WHERE reply_to_post_num IS NOT NULL AND timestamp > ?",
            (t30,),
        )
        replies30 = c2.fetchone()["n"]
        c2.execute(
            'SELECT AVG(LENGTH(json_extract(content, "$.text"))) as n FROM Posts WHERE timestamp > ?',
            (t30,),
        )
        ...
        plt.savefig(buf, format="png")
        buf.seek(0)
        images.append(("27_radar.png", buf))
        plt.close()
    except Exception as e:
    ...
    try:
        ...
        if data and len(data) >= 3:
            ...
            freshness = [
                (time.time() - row["last_ts"]) / 3600 for row in data
            ]  # hours ago
            ...
            ax.set_title(
                "28. Топ тредов (90д) — размер = активность",
                fontsize=12,
                fontweight="bold",
                color="#ffa657",
            )
            ...
            plt.savefig(buf, format="png")
            buf.seek(0)
            images.append(("28_threads_bubble.png", buf))
            plt.close()
    except Exception as e:
    ...
    try:
        ...
        if rows29:
            ...
            ax.stackplot(
                ...
                df29["med"],
                labels=["Текст", "Медиа"],
                colors=["#58a6ff", "#ff3399"],
                alpha=0.82,
            )
            step29 = max(1, len(df29) // 10)
            ax.set_xticks(xs29[::step29])
            ax.set_xticklabels(
                df29["d"].tolist()[::step29], rotation=30, ha="right", fontsize=7.5
            )
            ...
            plt.savefig(buf, format="png")
            buf.seek(0)
            images.append(("29_media_trend.png", buf))
            plt.close()
    except Exception as e:
    ...
    try:
        ...
        if data:
            ...
            avg_posts = [
                cohort[w]["posts"] / max(cohort[w]["new"], 1) for w in weeks_sorted
            ]
            ...
            ax1.set_xticklabels(
                [w.replace("20", "") for w in weeks_sorted], rotation=30, fontsize=7.5
            )
            ax1.set_ylabel("Новых авторов", color="#58a6ff")
            ax2.set_ylabel("Ср. постов на автора", color="#ffa657")
            ax1.set_title(
                "30. Когорты новых авторов (13 нед)",
                fontsize=13,
                fontweight="bold",
                color="#58a6ff",
            )
            ...
            plt.savefig(buf, format="png")
            buf.seek(0)
            images.append(("30_cohorts.png", buf))
            plt.close()
    except Exception as e:
    ...
    try:
        ...
        if _rows31:
            ...
            _df31["board_id"] = _df31["board_id"].apply(
                lambda b: b if b in _top_b31 else "other"
            )
            ...
            ax.stackplot(
                _xs31,
                [_piv31[b].values for b in _bords31],
                labels=_bords31,
                colors=_cl31[: len(_bords31)],
                alpha=0.85,
            )
            _step31 = max(1, len(_piv31) // 10)
            ax.set_xticks(_xs31[::_step31])
            ax.set_xticklabels(
                _piv31.index.tolist()[::_step31], rotation=30, ha="right", fontsize=8
            )
            ax.set_ylabel("Постов в неделю")
            ax.legend(loc="upper left", fontsize=9, framealpha=0.7)
            ax.set_title(
                "31. Активность борд по неделям (12 нед)",
                fontsize=13,
                fontweight="bold",
                color="#ffa657",
            )
            ...
            plt.savefig(buf, format="png")
            buf.seek(0)
            images.append(("31_boards_weekly.png", buf))
            plt.close()
    except Exception as e:
    ...
    try:
        ...
        if _rows32:
            ...
            for _uid32, _days32 in _ud32.items():
                ...
                for _i32 in range(1, len(_ds32)):
                    ...
                    if (_d1_32 - _d0_32).days == 1:
                        _cur32 += 1
                        _mx32 = max(_mx32, _cur32)
                    else:
                        _cur32 = 1
                _streaks32.append(
                    {"author_id": _uid32, "streak": _mx32, "days": len(_ds32)}
                )
            _streaks32 = sorted(_streaks32, key=lambda x: x["streak"], reverse=True)[
                :20
            ]
            ...
            for _ax32, _d32, _t32 in [
                (_ax32l, _df32l.iloc[::-1].reset_index(drop=True), "Топ 1–10"),
            ...
            plt.suptitle(
                "32. Стрик-чемпионы (60д) — самые стойкие аноны  Top-20",
                fontsize=14,
                fontweight="bold",
                color="#39d353",
                y=1.01,
            ...
            plt.savefig(buf, format="png", bbox_inches="tight")
            buf.seek(0)
            images.append(("32_streak_champions.png", buf))
            plt.close()
    except Exception as e:
...
def generate_user_stats_card(
    user_id: int, board_id: str, username: str
    ...
    c.execute(
        "SELECT balance, role, created_at, lie_media, custom_prefix FROM Users WHERE user_id = ? AND board_id = ?",
        (user_id, board_id),
    )
    profile = c.fetchone()
    if profile:
        ...
        balance, role, created_at, lie_media, custom_prefix = (
            0.0,
            "user",
            time.time(),
            0,
    ...
    c.execute(
        "SELECT COUNT(*) FROM Posts WHERE author_id = ? AND board_id = ?",
        (user_id, board_id),
    )
    ...
    # 5. Count mutes
    c.execute(
        "SELECT COUNT(*) FROM Mutes WHERE user_id = ? AND board_id = ?",
        (user_id, board_id),
    )
    ...
    if posts_count == 0:
        ...
        slang_comment = (
            "Почетный Скуф борды. Запах подпиваса и базированных мыслей за версту."
        )
    elif balance < 10:
        slang_comment = (
            "Нищук детектед. Проиграл все коины в рулетку или забанен за сажу."
        )
    else:
    ...
    try:
        ...
        font_title = font_subtitle = font_card_num = font_card_lbl = font_comment = (
    ...
    draw.rounded_rectangle(
        [610, 15, 770, 80], radius=6, fill="#1b1f28", outline="#ff9900", width=2
    )
    draw.text(
        (690, 33), "ТГАЧ CERTIFIED", fill="#ff9900", font=font_subtitle, anchor="mm"
    )
    ...
    def draw_card(x, y, w, h, val, label, color):
        draw.rounded_rectangle(
            [x, y, x + w, y + h], radius=6, fill="#13171f", outline="#252932", width=1
        )
        draw.ellipse([x + 15, y + 16, x + 23, y + 24], fill=color)
        draw.text(
            (x + 33, y + 20), label, fill="#969896", font=font_card_lbl, anchor="lm"
        )
    ...
    draw.rounded_rectangle(
        [600, 115, 770, 175], radius=6, fill="#1d1315", outline="#ff3333", width=1
    )
    ...
    draw.rounded_rectangle(
        [600, 210, 770, 290], radius=6, fill="#13171f", outline="#252932", width=1
    )
    ...
    draw.rounded_rectangle(
        [615, 255, 615 + int(140 * activity_pct), 267], radius=3, fill="#ff9900"
    )
    draw.text(
        (755, 230),
        f"{int(activity_pct*100)}%",
        fill="#ff9900",
        font=font_card_lbl,
        anchor="ra",
    )
    ...
    # Bottom Summary Box
    draw.rounded_rectangle(
        [30, 310, 770, 420], radius=8, fill="#1b1f28", outline="#252932", width=1
    )

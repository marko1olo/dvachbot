...
print(f"Found {len(file_ids)} matching file_ids from FileTagsFTS", flush=True)
...
if file_ids:
    ...
    t2 = time.time() - start
    print(f'New method returned {c2} posts in {t2*1000:.2f}ms', flush=True)

    clauses = []
    params = []
    for fid in file_ids[:10]: # test first 10 for speed
    ...
    t1 = time.time() - start
    print(f'Old method (10 file_ids) returned {c1} posts in {t1*1000:.2f}ms', flush=True)
conn.close()

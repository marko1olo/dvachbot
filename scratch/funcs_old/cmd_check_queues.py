...
async def cmd_check_queues(message: types.Message, board_id: str | None, stream: str = 'ru'):
    ...
    text = (
        f"≡ƒôè <b>╨í╨╛╤ü╤é╨╛╤Å╨╜╨╕╨╡ ╨╛╤ç╨╡╤Ç╨╡╨┤╨╡╨╣:</b>\n\n"
        f"≡ƒÜÇ <b>RAM (╨á╨░╤ü╤ü╤ï╨╗╨║╨░):</b> {ram_queue_size}\n"
        f"≡ƒº╡ <b>RAM total/top:</b> {queues.get('total', 0)} | <code>{escape_html(top_queue)}</code>\n"
        f"ΓÅ│ <b>Live age/current:</b> <code>{escape_html(live_queue_text)} | {escape_html(live_current_text)}</code>\n"
        f"≡ƒæÑ <b>Telegram recipients:</b> {recipients_snapshot.get('telegram_active_by_board', {}).get(board_id, '?')} on /{board_id}/; all {recipients_snapshot.get('telegram_active_total', '?')}\n"
        f"Γå⌐∩╕Å <b>Reply copies:</b> <code>{escape_html(reply_coverage_text)}</code>\n"
        f"ΓÜí <b>Priority active:</b> {priority_by_board.get(board_id, 0)} / {delivery_priority.get('total_weekly_active', 0)} ╨╖╨░ {delivery_priority.get('days', WEEKLY_ACTIVE_DAYS)}d split={delivery_priority.get('split_fanout')} slice={delivery_priority.get('passive_slice_size')}/{delivery_priority.get('passive_media_slice_size')} pressure>={delivery_priority.get('pressure_slice_age_sec')}s:{delivery_priority.get('pressure_passive_slice_size')}/{delivery_priority.get('pressure_passive_media_slice_size')} priority_budget={delivery_priority.get('priority_phase_budget_sec')}s passive_budget={delivery_priority.get('passive_phase_budget_sec')}s guard={delivery_priority.get('delivery_phase_guard_sec')}s preempt={delivery_priority.get('passive_max_preemptions')} chunk={delivery_priority.get('delivery_initial_chunk_size')}/{delivery_priority.get('delivery_min_chunk_size')} uid_timeout={delivery_priority.get('delivery_per_recipient_timeout_sec')}s uid_retries={delivery_priority.get('delivery_max_recipient_retries')}\n"
        f"≡ƒº╖ <b>Durable delivery:</b> enabled={durable_delivery_snapshot.get('enabled')} DB pending={db_stats.get('delivery', 0)} saved={durable_delivery_snapshot.get('persisted', 0)} fail={durable_delivery_snapshot.get('persist_failed', 0)} restored={durable_delivery_snapshot.get('restored_items', 0)}/{durable_delivery_snapshot.get('restored_recipients', 0)} deleted={durable_delivery_snapshot.get('deleted', 0)}\n"
        f"≡ƒû╝ <b>Anime media:</b> conc={anime_media_snapshot.get('concurrency')} b_max={anime_media_snapshot.get('b_max_stacked_images')} url={anime_media_snapshot.get('url_parallel')}x/{anime_media_snapshot.get('url_timeout_sec')}s total={anime_media_snapshot.get('url_total_sec')}s dl={anime_media_snapshot.get('download_parallel')}x/{anime_media_snapshot.get('download_timeout_sec')}s\n"
        f"≡ƒÄ¡ <b>Mode punch-up:</b> runtime={mode_punchup_snapshot.get('runtime_enabled')} shed={mode_punchup_snapshot.get('queue_shed_sec')}s calls={mode_punchup_stats.get('calls', 0)} skip_load={mode_punchup_stats.get('skipped_load', 0)}\n"
        f"≡ƒÆ¼ <b>Context replies:</b> enabled={contextual_snapshot.get('enabled')} groups={contextual_snapshot.get('groups_ru')} tracked={contextual_snapshot.get('tracked_users')} sent={contextual_stats.get('sent', 0)} skip_cd/daily={contextual_stats.get('skipped_cooldown', 0)}/{contextual_stats.get('skipped_daily_limit', 0)} cd={contextual_snapshot.get('cooldown_sec')}s limit={contextual_snapshot.get('daily_limit')}\n"
        f"≡ƒô¿ <b>Last delivery:</b> <code>{escape_html(last_delivery_text)}</code> avg/max <code>{board_delivery.get('avg_sec', 0)} / {board_delivery.get('max_sec', 0)}s</code>\n"
        f"≡ƒÆ╛ <b>DB (Broadcast):</b> {db_stats.get('broadcast', 0)}\n"
        f"≡ƒöö <b>DB (╨ú╨▓╨╡╨┤╨╛╨╝╨╗╨╡╨╜╨╕╤Å):</b> {db_stats.get('notif', 0)}\n"
        f"≡ƒ¬₧ <b>DB (╨ù╨╡╤Ç╨║╨░╨╗╨░ ╤ä╨░╨╣╨╗╨╛╨▓):</b> {db_stats.get('mirror', 0)}\n"
        f"≡ƒæ« <b>DB (╨£╨╛╨┤╨╡╤Ç╨░╤å╨╕╤Å):</b> {db_stats.get('mod', 0)}\n"
        f"≡ƒºá <b>RSS/private:</b> {memory.get('rss_mb', '?')} / {memory.get('private_mb', '?')} MB"
    )

...
async def cmd_deanon(message: Message, board_id: str | None, stream: str = 'ru'):
    ...
    # storage_lock ╤â╨▒╤Ç╨░╨╜: ╨║╤â╨╗╨┤╨░╤â╨╜ ╨▓ board_data, ╨╕╤ü╨║╨╗╤Ä╤ç╨╡╨╜╨╕╨╡ ╨┤╨░╤æ╤é deanon_lock.
    ...
    if not message.reply_to_message:
        reply_text = "≡ƒæÇ Reply to a message to de-anonymize!" if lang == 'en' else "ΓÜá∩╕Å ╨₧╤é╨▓╨╡╤é╤î╤é╨╡ ╨╜╨░ ╨░╨╜╨╛╨╜╨╕╨╝╨╜╨╛╨╡ ╤ü╨╛╨╛╨▒╤ë╨╡╨╜╨╕╨╡ ╤Ä╨╖╨╡╤Ç╨░, ╤ç╤é╨╛╨▒╤ï ╨┐╨╛╨┐╤ï╤é╨░╤é╤î╤ü╤Å ╤â╨╖╨╜╨░╤é╤î ╨░╨▓╤é╨╛╤Ç╨░: <code>/deanon</code>"
        await message.answer(reply_text, parse_mode="HTML")
        await _safe_delete_user_message(message)
        return
    user_id = message.from_user.id
    b_data = board_data[board_id] # ╨ƒ╨╡╤Ç╨╡╨╛╨┐╤Ç╨╡╨┤╨╡╨╗╨╡╨╜╨╕╨╡ b_data ╨┤╨╗╤Å ╤Å╤ü╨╜╨╛╤ü╤é╨╕
    user_location = 'main'
    ...
    if not original_author_id:
        reply_text = "≡ƒÜ½ Could not find the post to de-anonymize..." if lang == 'en' else "≡ƒÜ½ ╨¥╨╡ ╤â╨┤╨░╨╗╨╛╤ü╤î ╨╜╨░╨╣╤é╨╕ ╨┐╨╛╤ü╤é ╨┤╨╗╤Å ╨┤╨╡╨░╨╜╨╛╨╜╨░..."
        await message.answer(reply_text)
        await _safe_delete_user_message(message)
        return
    if original_author_id == 0:
        reply_text = "ΓÜá∩╕Å System messages cannot be de-anonymized." if lang == 'en' else "ΓÜá∩╕Å ╨í╨╕╤ü╤é╨╡╨╝╨╜╤ï╨╡ ╤ü╨╛╨╛╨▒╤ë╨╡╨╜╨╕╤Å ╨╜╨╡╨╗╤î╨╖╤Å ╨┤╨╡╨░╨╜╨╛╨╜╨╕╤é╤î."
        await message.answer(reply_text)
        await _safe_delete_user_message(message)
        return
    deanon_text = generate_deanon_info(lang=lang)
    header_text_prefix = "### DEANON ###" if lang == 'en' else "### ╨ö╨ò╨É╨¥╨₧╨¥ ###"
    ...
    async def create_and_send_deanon_post(thread_id_override=None):
        ...
        if not pnum:
            print(f"Γ¢ö [{board_id}] ╨Ü╨á╨ÿ╨ó╨ÿ╨º╨ò╨í╨Ü╨É╨» ╨₧╨¿╨ÿ╨æ╨Ü╨É: ╨╜╨╡ ╤â╨┤╨░╨╗╨╛╤ü╤î ╤ü╨╛╨╖╨┤╨░╤é╤î ╨┐╨╛╤ü╤é ╨▓ ╨æ╨ö ╨┤╨╗╤Å /deanon.")
            return

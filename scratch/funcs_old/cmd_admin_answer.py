...
async def cmd_admin_answer(message: types.Message, board_id: str | None, stream: str = 'ru'):
    """
    ╨₧╤é╨┐╤Ç╨░╨▓╨╗╤Å╨╡╤é ╤ü╨╕╤ü╤é╨╡╨╝╨╜╤ï╨╣ ╨╛╤é╨▓╨╡╤é ╨╜╨░ ╨┐╨╛╤ü╤é ╨┐╨╛╨╗╤î╨╖╨╛╨▓╨░╤é╨╡╨╗╤Å.
    """
    if not board_id or not is_admin(message.from_user.id, board_id): return
    lang = stream if ENABLE_MULTILANG else ('en' if board_id == 'int' else 'ru')
    if not message.reply_to_message:
        err = "Use as reply: /ans &lt;text&gt;" if lang == 'en' else ("Φ┐öΣ┐íπü¿πüùπüªΣ╜┐τö¿πüùπüªπüÅπüáπüòπüä: /ans &lt;text&gt;" if lang == 'jp' else "ΓÜá∩╕Å ╨₧╤é╨▓╨╡╤é╤î╤é╨╡ ╨╜╨░ ╤ü╨╛╨╛╨▒╤ë╨╡╨╜╨╕╨╡ ╤Ä╨╖╨╡╤Ç╨░: <code>/ans &lt;╨╛╤ä╨╕╤å╨╕╨░╨╗╤î╨╜╤ï╨╣ ╨╛╤é╨▓╨╡╤é ╨░╨┤╨╝╨╕╨╜╨░&gt;</code>")
    ...
    if not answer_text:
        err = "Enter answer text." if lang == 'en' else ("σ¢₧τ¡öπéÆσàÑσè¢πüùπüªπüÅπüáπüòπüäπÇé" if lang == 'jp' else "╨Æ╨▓╨╡╨┤╨╕╤é╨╡ ╤é╨╡╨║╤ü╤é ╨╛╤é╨▓╨╡╤é╨░.")

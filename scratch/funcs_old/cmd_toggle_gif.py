...
async def cmd_toggle_gif(message: types.Message, board_id: str | None, stream: str = 'ru'):
    ...
    act = "╨ù╨É╨ƒ╨á╨ò╨ó╨ÿ╨¢ GIF" if new_val else "╨á╨É╨ù╨á╨ò╨¿╨ÿ╨¢ GIF"
    await log_global_event('bot', f"≡ƒû╝∩╕Å GIF_TOGGLE: ╨É╨┤╨╝╨╕╨╜ {message.from_user.id} {act} ╨┐╨╛╨╗╤î╨╖╨╛╨▓╨░╤é╨╡╨╗╤Ä {target_id} ╨╜╨░ /{board_id}/")
    if lang == 'en':
        ...
        msg = f"{target_id} πü«GIF: {status}"
    else:
        status = "╨ù╨É╨ƒ╨á╨ò╨⌐╨ò╨¥╨½ ≡ƒÜ½ (╨ó╨╡╨╜╨╡╨▓╨╛╨╣)" if new_val else "╨á╨É╨ù╨á╨ò╨¿╨ò╨¥╨½ Γ£à"
        msg = f"╨ô╨╕╤ä╨║╨╕ ╨┤╨╗╤Å {target_id} ╤é╨╡╨┐╨╡╤Ç╤î: {status}"

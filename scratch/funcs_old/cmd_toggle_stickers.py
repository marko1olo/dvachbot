...
async def cmd_toggle_stickers(message: types.Message, board_id: str | None, stream: str = 'ru'):
    ...
    act = "╨ù╨É╨ƒ╨á╨ò╨ó╨ÿ╨¢ ╤ü╤é╨╕╨║╨╡╤Ç╤ï" if new_val else "╨á╨É╨ù╨á╨ò╨¿╨ÿ╨¢ ╤ü╤é╨╕╨║╨╡╤Ç╤ï"
    await log_global_event('bot', f"≡ƒâÅ STICK_TOGGLE: ╨É╨┤╨╝╨╕╨╜ {message.from_user.id} {act} ╨┐╨╛╨╗╤î╨╖╨╛╨▓╨░╤é╨╡╨╗╤Ä {target_id} ╨╜╨░ /{board_id}/")
    if lang == 'en':
        ...
        msg = f"{target_id} πü«πé╣πâåπââπé½πâ╝: {status}"
    else:
        status = "╨ù╨É╨ƒ╨á╨ò╨⌐╨ò╨¥╨½ ≡ƒÜ½ (╨ó╨╡╨╜╨╡╨▓╨╛╨╣)" if new_val else "╨á╨É╨ù╨á╨ò╨¿╨ò╨¥╨½ Γ£à"
        msg = f"╨í╤é╨╕╨║╨╡╤Ç╤ï ╨┤╨╗╤Å {target_id} ╤é╨╡╨┐╨╡╤Ç╤î: {status}"

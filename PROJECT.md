...
## Architecture
- **Dedicated Single-Bot Pipeline**: In Telegram, bots can ONLY send direct messages to users who have initiated a PM conversation (`/start`) with that specific bot. 95%+ of traffic is on `/b/` via `@dvach_chatbot`. Multi-bot cross-broadcasting across board bots is strictly invalid and causes 100% 403 Forbidden errors. Optimization must maximize the delivery pipeline for each board's dedicated bot to reach Telegram's physical ceiling of 25-30 msg/sec per bot.
- **Two-Phase Delivery Model**:
  - **Priority Phase**: Dispatches immediately to weekly active users ($N_{priority} \approx 90\text{--}100$) in $< 2.0\text{s}$ using high-concurrency chunks.
  - **Passive Slicing Phase**: Batches remaining passive users into optimized slices (250-300 text, 120-150 media) with adaptive rate limiting (28-30 tokens/sec) and zero artificial lag.
- **Durable Persistence**: `DeliveryQueue` and `PostCopies` in SQLite persist remaining passive slices across process restarts. Restored tasks accurately resume without duplicates or message drops.
...
| 4 | R3: High-Throughput Chunk & Rate Calibration | Increase chunk size to 25-30, calibrate passive slice sizes (250-300 text / 120-150 media), remove artificial sleep bottlenecks | M2 | ORIGINAL_REQUEST §R3 |
...
| 2 | M2: Safe High-Throughput Delivery Pipeline Optimization | Slicing calibration, chunk size optimization, rate limiter, and priority scheduling | M1 | IN_PROGRESS |
| 3 | M3: Automated Verification & Acceptance Gate | Concurrency benchmarking (1000+ users), performance validation, and py_compile check | M2 | PLANNED |

...
def format_sys_value(value, unit="%"):
    if not isinstance(value, (int, float)):
        return "[dim]N/A[/dim]"
    if value > 90:
        return f"[bold red]{value:.1f}{unit}[/bold red]"
    if value > 75:
        return f"[bold yellow]{value:.1f}{unit}[/bold yellow]"
...
>>>>>>> 5315ade (⚡ Optimize get_activity by batching async queries)
    }
    queries = []
    names = []
    for name, query in queue_map.items():
        queries.append(query)
        names.append(name)

    cursors = await asyncio.gather(
        *(conn.execute(q) for q in queries), return_exceptions=True
    )
    ...
    for i, name in enumerate(names):
        ...
        if isinstance(c, Exception):
            ...
            try:
                ...
                count, oldest_ts = row if row else (0, 0)
    ...
    for name, ts in periods.items():
        queries.extend(
            [
                ...
                conn.execute(
                    "SELECT COUNT(*) FROM Threads WHERE created_at > ?", (ts,)
                ),
                conn.execute(
                    "SELECT COUNT(DISTINCT user_id) FROM Users WHERE created_at > ?",
                    (ts,),
                ),
            ]
        )
        keys.extend([f"posts_{name}", f"threads_{name}", f"users_{name}"])

    try:
        cursors = await asyncio.gather(*queries)
        results = await asyncio.gather(*(c.fetchone() for c in cursors))
...
async def get_media_stats(conn):
    ...
    try:
        type_cursor = await conn.execute(
            "SELECT file_type, COUNT(*) FROM FileRegistry GROUP BY file_type"
        )
...
async def get_top_activity(conn):
    ...
    top = {}
    try:
        boards_cursor = await conn.execute(
            "SELECT board_id, COUNT(*) as c FROM Posts WHERE timestamp > ? GROUP BY board_id ORDER BY c DESC LIMIT 5",
            (day_ago,),
        )
        threads_cursor = await conn.execute(
            "SELECT thread_id, board_id, title, reply_count FROM Threads WHERE is_archived = 0 ORDER BY last_updated_at DESC LIMIT 5"
        )
...
async def main():
    if not os.path.exists(DB_NAME):
        console.print(
            f"[bold red]Ошибка: Файл базы данных '{DB_NAME}' не найден![/bold red]"
        )
        return

    console.rule(
        f"[bold cyan]Отчет о состоянии системы TGACH ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')})[/bold cyan]"
    )
    ...
    async with aiosqlite.connect(DB_NAME) as conn:
        ...
        for name, d in queues.items():
            q_table.add_row(
                name, format_queue_value(d["count"]), format_age(d["oldest"])
            )
        ...
        a_table.add_row(
            "За час",
            f"{activity.get('posts_1h', 0):,}",
            f"{activity.get('threads_1h', 0):,}",
            f"{activity.get('users_1h', 0):,}",
        )
        a_table.add_row(
            "За 24 часа",
            f"{activity.get('posts_24h', 0):,}",
            f"{activity.get('threads_24h', 0):,}",
            f"{activity.get('users_24h', 0):,}",
        )
        ...
        if total > 0:
            ...
            m_table.add_row(
                "  - с тегами (Tags)", f"{tagged:,}", f"{tagged / total * 100:.1f}%"
            )
            m_table.add_row(
                "  - с перцептивным хешем (pHash)",
                f"{phash:,}",
                f"{phash / total * 100:.1f}%",
            )
            m_table.add_row(
                "  - с заглушкой (BlurHash)", f"{blur:,}", f"{blur / total * 100:.1f}%"
            )
            m_table.add_section()
            m_table.add_row("[bold]Разбивка по типам[/bold]", "", "")

            sorted_types = sorted(
                media_stats.get("by_type", {}).items(), key=lambda i: i[1], reverse=True
            )
            ...
            m_table.add_section()
            m_table.add_row(
                "[bold]Всего превью[/bold]",
                f"{media_stats.get('total_thumbnails', 0):,}",
                "",
            )
        else:
        ...
        b_text = "\n".join(
            [
                f"{i+1}. [bold]/{b[0]}[/bold] ({b[1]:,} постов)"
                for i, b in enumerate(top.get("boards", []))
            ]
        )
        t_text = "\n".join(
            [
                f"{i+1}. [bold]/{t[1]}/ → #{t[0]}[/bold] ({t[3]} replies)\n   '{t[2][:40]}...'\n"
        ...
        # --- Системные панели ---
        cpu, ram, disk = (
            format_sys_value(sys_health.get("cpu", 0)),
            format_sys_value(sys_health.get("ram", 0)),
            format_sys_value(sys_health.get("disk", 0)),
        )
        db_size = db_sizes.get(DB_NAME, "N/A")
        sys_panel = Panel(
            Text.from_markup(
                f" [bold]CPU:[/bold] {cpu} | [bold]RAM:[/bold] {ram} | [bold]Disk:[/bold] {disk} | [bold]DB Size:[/bold] {db_size}"
            ),
            title="💻 Система",
            border_style="green",
        )

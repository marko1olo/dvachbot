...
@pytest.mark.parametrize("name,sec_module", [
    ("dubsite", dubsite_sec),
    ("site", site_sec),
])
def test_first_request(name, sec_module):
    ip = "1.2.3.4"
    with patch.object(sec_module.random, 'random', return_value=1.0):
        result = sec_module.check_ddos(ip)
        assert not result
        assert ip in sec_module.REQUEST_HISTORY
        assert sec_module.REQUEST_HISTORY[ip][0] == 1

@pytest.mark.parametrize("name,sec_module", [
    ("dubsite", dubsite_sec),
    ("site", site_sec),
])
def test_multiple_requests_under_limit(name, sec_module):
    ip = "1.2.3.4"
    with patch.object(sec_module.random, 'random', return_value=1.0):
        ...
        result = sec_module.check_ddos(ip)
        assert not result
        assert sec_module.REQUEST_HISTORY[ip][0] == 3

@pytest.mark.parametrize("name,sec_module", [
    ("dubsite", dubsite_sec),
    ("site", site_sec),
])
def test_exceeding_limit_bans_ip(name, sec_module):
    ip = "1.2.3.4"
    with patch.object(sec_module.random, 'random', return_value=1.0):
        for _ in range(sec_module.MAX_REQUESTS_PER_WINDOW + 1):
            result = sec_module.check_ddos(ip)
        assert result
        assert ip in sec_module.IP_BAN_LIST
        assert ip not in sec_module.REQUEST_HISTORY

@pytest.mark.parametrize("name,sec_module", [
    ("dubsite", dubsite_sec),
    ("site", site_sec),
])
def test_already_banned_ip(name, sec_module):
    ip = "1.2.3.4"
    now = time.time()
...
def test_ban_expired(name, sec_module):
    ip = "1.2.3.4"
    now = time.time()
    ...
    with patch.object(sec_module.random, 'random', return_value=1.0), \
        ...
        result = sec_module.check_ddos(ip)
        assert not result
        assert ip not in sec_module.IP_BAN_LIST
        assert ip in sec_module.REQUEST_HISTORY
...
def test_probabilistic_cleanup(name, sec_module):
    ip = "1.2.3.4"
    now = time.time()
    ...
    with patch.object(sec_module.random, 'random', return_value=0.001), \
        ...
        assert expired_ip not in sec_module.REQUEST_HISTORY

@pytest.mark.parametrize("name,sec_module", [
    ("dubsite", dubsite_sec),
    ("site", site_sec),
])
def test_max_history_cleanup(name, sec_module):
    now = time.time()
    ...
    with patch.object(sec_module.random, 'random', return_value=1.0), \
        ...
        assert len(sec_module.REQUEST_HISTORY) <= sec_module.MAX_HISTORY_SIZE

@pytest.mark.parametrize("name,sec_module", [
    ("dubsite", dubsite_sec),
    ("site", site_sec),
])
def test_window_expiration_resets_count(name, sec_module):
    ip = "1.2.3.4"
    now = time.time()
    ...
    with patch.object(sec_module.random, 'random', return_value=1.0), \
        ...
        assert not result
        assert sec_module.REQUEST_HISTORY[ip][0] == 1
        assert sec_module.REQUEST_HISTORY[ip][1] == now

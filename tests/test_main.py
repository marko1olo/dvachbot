...
def get_clean_html_tags_function():
    import ast
    import os
    ...
    module = ast.parse(source)
    for node in module.body:
        if isinstance(node, ast.FunctionDef) and node.name == 'clean_html_tags':
            code = compile(ast.Module(body=[node], type_ignores=[]), filename="<ast>", mode="exec")
            namespace = {
                're': __import__('re'),
                'RE_HTML_TAGS': __import__('re').compile(r'<[^>]+>')
            }
            exec(code, namespace)
            return namespace['clean_html_tags']
    return None

clean_html_tags = get_clean_html_tags_function()
...
class TestCleanHtmlTags(unittest.TestCase):
    ...
    def test_no_tags(self):
        self.assertEqual(clean_html_tags("Hello World"), "Hello World")

    def test_simple_tags(self):
        self.assertEqual(clean_html_tags("<b>Hello</b> World"), "Hello World")
        self.assertEqual(clean_html_tags("<p>Test</p>"), "Test")

    def test_multiple_tags(self):
        self.assertEqual(clean_html_tags("<div><span>Text</span></div>"), "Text")
        self.assertEqual(clean_html_tags("<a href='link'>Link</a>"), "Link")

    def test_malformed_tags(self):
        self.assertEqual(clean_html_tags("< b >Hello< /b >"), "Hello")
        self.assertEqual(clean_html_tags("<bHello"), "<bHello")

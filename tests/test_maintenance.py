...
class TestMaintenance(unittest.TestCase):
    ...
    def test_run_maintenance_db_not_found(self, mock_connect, mock_print, mock_exists):
        ...
        mock_print.assert_called_once_with(
            f"Ошибка: Файл базы данных не найден по пути: {DB_NAME}"
        )
    ...
    def test_run_maintenance_exception(self, mock_connect, mock_print, mock_exists):
        ...
        mock_print.assert_any_call(
            f"⛔ КРИТИЧЕСКАЯ ОШИБКА во время обслуживания: {error_msg}"
        )

    @patch("maintenance.os.path.exists")
    @patch("builtins.print")
    @patch("maintenance.sqlite3.connect")
    def test_run_maintenance_execute_exception(
        self, mock_connect, mock_print, mock_exists
    ):
        """Test error condition: An exception is raised during query execution."""
        mock_exists.return_value = True
        ...
        mock_con.execute.side_effect = Exception(error_msg)

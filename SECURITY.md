# 🔒 Security Policy — marko1olo/dvachbot
...
> Project Scope: `TGACH & dvachbot Imageboard Streaming Engine & Atkinson Bridge`
...
We actively maintain and provide critical security updates for the following release lines of **marko1olo/dvachbot**:
...
Security engineering in marko1olo/dvachbot is guided by the following domain-specific threat vector analyses:

### 1. SSRF in Image Fetcher
* **Description**: Media scrapers fetching AWS metadata endpoints (169.254.169.254) via malicious links.
...
### 2. Decompression Bomb Exploits
* **Description**: Huge pixel dimensions (e.g. 50,000x50,000) triggering out-of-memory crashes.
* **Impact Rating**: HIGH / CRITICAL
* **Mitigation Strategy**: Strict schema validation, boundary fuzz testing, and automated static security analysis.
### 3. Command Injection in Media Pipelines
* **Description**: Unescaped filenames passed directly into subprocess transcode pipes.
* **Impact Rating**: HIGH / CRITICAL
* **Mitigation Strategy**: Strict schema validation, boundary fuzz testing, and automated static security analysis.
...
If you discover a security flaw or exploit vector in **marko1olo/dvachbot**, do **NOT** post it publicly in open issues or discussions.

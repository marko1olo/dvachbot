"""
Backfill Pixhost mirrors for all registered image files.
Persistent loop - runs until killed.
Usage: python scripts/backfill_pixhost.py
"""
import asyncio
import contextlib
import os
...
async def main():
    flush_print("[RUN] Starting Pixhost Backfill Script (Persistent Loop Edition)...")
        ...
        try:
            with contextlib.closing(sqlite3.connect(DB_PATH)) as db:
                ...
                c.execute("""
                    ...
                    FROM FileRegistry r
                    LEFT JOIN FileMirrors m ON r.file_id = m.file_id AND m.mirror_type = 'pixhost'
            ...
            if not rows:
                flush_print(f"[SUCCESS] No missing mirrors for Pixhost. Sleeping for {SLEEP_ON_EMPTY} seconds...")
                await asyncio.sleep(SLEEP_ON_EMPTY)
                continue

            flush_print(f"[INFO] Found {len(rows)} pending image files for backfill. Starting batch processing...")
            ...
            for idx, (file_id, file_type) in enumerate(rows):
                ...
                try:
                    ...
                    if not bot:
                        flush_print(f"[WARN] No active bot found for file {file_id[:15]}. Skipping.")
                        skipped_count += 1
                        continue
                    ...
                    if file_size > PIXHOST_MAX_MB * 1024 * 1024:
                        flush_print(f"[SKIP] File size too large ({file_size/1024/1024:.1f} MB). Max 10MB on Pixhost. Skipping.")
                        skipped_count += 1
                        continue
                    ...
                    if file_path_tg:
                        ...
                        if file_type == 'animation':
                            flush_print(f"[SKIP] Format animation is not supported by Pixhost. Skipping.")
                            skipped_count += 1
                            continue
                        if e.lower() in PIXHOST_SUPPORTED_EXT:
                            ext = e.lower()

                    fd, lpath = tempfile.mkstemp(prefix='pixhost_bf_', suffix=ext)
                    os.close(fd)
                    ...
                    try:
                        ...
                        flush_print(f"[INFO] Uploading to Pixhost...")
                        link = await upload_file_to_pixhost(lpath)
                        if link:
                            await add_file_mirror(file_id, 'pixhost', link)
                            flush_print(f"[SUCCESS] Mirrored to Pixhost: {link}")
                            success_count += 1
                        else:
                            flush_print(f"[ERROR] Pixhost upload failed.")
                            failed_count += 1

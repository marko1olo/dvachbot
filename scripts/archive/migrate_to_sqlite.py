    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        for board_id, config in BOARD_CONFIG.items():
            await db.execute(
                ...
                (board_id, config.get('name', ''), config.get('description', ''))
            )
        await db.commit()
                ...
                try:
                    ...
                    if expiry_dt > datetime.now(UTC):
                        await db.execute(
                            ...
                            VALUES (?, ?, ?, ?)
                            """,
                            (user_id, board_id, 'shadow', expiry_dt.timestamp())
                        )
                        total_mutes += 1
                ...
                for user_id_str, state in user_states.items():
                    ...
                    if location != 'main':
                        await db.execute(
                            ...
                            (location, int(user_id_str), board_id)
                        )
                        total_user_states += 1
        await db.commit()

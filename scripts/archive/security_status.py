...
def _evaluate_blockers(status: dict[str, Any]) -> list[dict[str, Any]]:
    strict_blockers: list[dict[str, Any]] = []

    if not status["source_scan_ok"]:
        add_blocker(strict_blockers, "source_scan_not_ok", 1, "latest security_check summary is not ok")
    if not status["source_integrity_exists"]:
        add_blocker(strict_blockers, "source_integrity_missing", 1, "source integrity report is missing")
    add_blocker(strict_blockers, "source_integrity_issues", status["source_integrity_issue_count"], "Python source integrity issues")
    if not status["repository_health_exists"]:
        add_blocker(strict_blockers, "repository_health_missing", 1, "repository health report is missing")
    add_blocker(strict_blockers, "repository_health_issues", status["repository_health_issue_count"], "Git metadata/status issues")
    if not status["gitignore_policy_exists"]:
        add_blocker(strict_blockers, "gitignore_policy_missing", 1, "gitignore policy report is missing")
    add_blocker(strict_blockers, "gitignore_missing_patterns", status["gitignore_missing_count"], "required local-secret ignore patterns missing")
    if not status["cookie_artifacts_exists"]:
        add_blocker(strict_blockers, "cookie_artifacts_missing", 1, "cookie artifact report is missing")
    add_blocker(strict_blockers, "cookie_sensitive_files", status["cookie_sensitive_file_count"], "sensitive cookie jars remain local")
    add_blocker(strict_blockers, "cookie_parse_issues", status["cookie_parse_issue_count"], "malformed cookie artifact files")
    if not status["local_artifacts_exists"]:
        add_blocker(strict_blockers, "local_artifacts_missing", 1, "local artifact report is missing")
    add_blocker(strict_blockers, "local_sensitive_artifacts", status["local_artifact_count"], "local env/db/log/archive/session artifacts remain")
    if not status["python_quality_exists"]:
        add_blocker(strict_blockers, "python_quality_missing", 1, "Python quality report is missing")
    add_blocker(strict_blockers, "python_quality_critical_findings", status["python_quality_critical_count"], "critical Python file/function size findings")
    if not status["import_graph_exists"]:
        add_blocker(strict_blockers, "import_graph_missing", 1, "import graph report is missing")
    add_blocker(strict_blockers, "import_graph_cycles", status["import_graph_cycle_count"], "project import cycles")
    if not status["async_blocking_exists"]:
        add_blocker(strict_blockers, "async_blocking_missing", 1, "async blocking report is missing")
    add_blocker(strict_blockers, "async_blocking_high_findings", status["async_blocking_high_count"], "high-risk blocking calls inside async functions")
    if not status["env_contract_exists"]:
        add_blocker(strict_blockers, "env_contract_missing", 1, "env contract report is missing")
    add_blocker(strict_blockers, "env_contract_missing_keys", status["env_contract_missing_count"], "used env keys missing from .env.example")
    add_blocker(strict_blockers, "env_contract_dynamic_refs", status["env_contract_dynamic_reference_count"], "dynamic env references remain")
    add_blocker(strict_blockers, "remediation_actions", status["remediation_actions"], "manual secret remediation actions remain")
    add_blocker(strict_blockers, "validator_issues", len(status["validator_issues"]), "security report validator issues")
...
def build_status(include_summary_validation: bool = True) -> dict[str, Any]:
    ...
    strict_blockers = _evaluate_blockers(status)
    status["strict_blocker_count"] = len(strict_blockers)
    status["strict_blockers"] = strict_blockers
    status["strict_ready"] = len(strict_blockers) == 0

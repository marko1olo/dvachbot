"""
Enqueue the latest 10,000 unmirrored post images into MirrorQueue for Pixhost.
...
# Query the latest 10,000 un-mirrored post images
query = """
    ...
    WHERE pf.original_file_id LIKE 'AgAC%'
      AND NOT EXISTS (
          ...
          WHERE fm.file_id = pf.original_file_id AND fm.mirror_type = 'pixhost'
      )
      AND NOT EXISTS (
          SELECT 1 FROM MirrorQueue mq 
          WHERE mq.file_id = pf.original_file_id AND mq.mirror_type = 'pixhost'
      )
    ORDER BY p.post_num DESC
    LIMIT ?
"""

print(f"Selecting top {LIMIT_COUNT:,} latest un-mirrored files for Pixhost...")
c.execute(query, (LIMIT_COUNT,))
rows = c.fetchall()
print(f"Found {len(rows):,} candidate files.")

if not rows:
    print("All recent files are already queued or mirrored!")
    conn.close()
    sys.exit(0)
...
print(f"\n✅ Successfully enqueued {inserted:,} files into MirrorQueue for 'pixhost'!")
print("The background mirror_worker will automatically process them concurrently (20 threads).")

...
def analyze_visitors_log(filepath):
    ...
    ip_stats = defaultdict(lambda: {"requests": 0, "is_live": False, "country": "Unknown", "endpoints": defaultdict(int)})
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if action_type == "ENTER":
                ...
                unique_ips.add(ip)
                if action_detail:
                    if action_detail.startswith("GET ") or action_detail.startswith("POST "):
    ...
    sorted_endpoints = sorted(top_endpoints.items(), key=lambda x: x[1], reverse=True)[:10]

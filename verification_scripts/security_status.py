...
def _load_metrics(include_summary_validation: bool) -> dict[str, Any]:
...
def _evaluate_blockers(metrics: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    add_blocker(strict_blockers, "python_quality_critical_findings", metrics["python_quality_critical_count"], "critical Python file/function size findings")
    ...
    add_blocker(strict_blockers, "env_contract_dynamic_refs", metrics["env_contract_dynamic_count"], "dynamic env references remain")
    add_blocker(strict_blockers, "remediation_actions", metrics["remediation_action_count"], "manual secret remediation actions remain")
...
def build_status(include_summary_validation: bool = True) -> dict[str, Any]:
    metrics = _load_metrics(include_summary_validation)
    strict_blockers = _evaluate_blockers(metrics)
    ...
    return {
        ...
        "cookie_artifact_file_count": int(metrics["cookie_artifacts"].get("cookie_file_count", 0)),
        ...
        "local_artifact_total_size_bytes": int(metrics["local_artifacts"].get("total_size_bytes", 0)),
        "python_quality_exists": metrics["python_quality_exists"],
        "python_quality_large_file_count": int(metrics["python_quality"].get("large_file_count", 0)),
        "python_quality_critical_file_count": metrics["python_quality_critical_file_count"],
        "python_quality_flagged_function_count": int(metrics["python_quality"].get("flagged_function_count", 0)),
        "python_quality_critical_function_count": metrics["python_quality_critical_function_count"],
        "python_quality_critical_count": metrics["python_quality_critical_count"],
        "import_graph_exists": metrics["import_graph_exists"],
        "import_graph_module_count": int(metrics["import_graph"].get("module_count", 0)),
        ...
        "async_blocking_finding_count": int(metrics["async_blocking"].get("finding_count", 0)),
        ...
        "inventory_secret_like_count": int(metrics["inventory"].get("total_secret_like_count", 0)),
        "baseline_total_secret_like_count": int(metrics["baseline"].get("total_secret_like_count", 0)),
        "remediation_actions": metrics["remediation_action_count"],
        "destructive_action_performed": bool(metrics["remediation"].get("destructive_action_performed", False)),
        "validator_ok": not metrics["validator_issues"],
        "validator_issues": metrics["validator_issues"],

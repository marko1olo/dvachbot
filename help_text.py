...
HELP_HUB_PAGES_JP = {
    ...
    "modes": (
        ...
        "• 🐊 <code>/rus</code> — 古代スラヴ戦士 vs トカゲ星人 v6.0（バギロフ教授講義・バイカル聖水）\n"
        "• 🕶 <code>/matrix</code> — マトリックス風\n"
        "• 🦅 <code>/america</code> — アメリカ風\n"
        "• ⏹️ <code>/stop</code> — モード停止\n\n"
        "🐊 <b>スラヴ戦士 vs トカゲ — サブコマンド:</b>\n"
        "• 🎓 <code>/bagirov</code> (/багиров) — バギロフ教授の講義\n"
        "• ⚔️ <code>/drocheslav</code> (/дрочеслав) — マンモス騎兵隊の襲撃\n"
        "• 🧬 <code>/yashchertest</code> (/ящертест) — スラヴ血統DNAテスト\n"
        "• ⚖️ <code>/veche</code> (/вече) — ノヴゴロド民会裁判\n"
        "• 🌌 <code>/horoscope_rus</code> (/чертог) — スラヴ星座占い\n"
        "• 📜 <code>/slav_trade</code> (/грамота) — ノヴゴロド商人の証書\n"
        "• 🚀 <code>/hyperborea</code> (/гиперборея) — ヒュペルボレア年代記\n"
        "• ⚡ <code>/slav_curse</code> (/анафема) — トカゲへの呪い"
    ),
    "actions": (
